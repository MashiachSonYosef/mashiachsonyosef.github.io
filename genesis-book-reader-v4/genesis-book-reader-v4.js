(() => {
  ...
  // Override data (recorded in synthesis/gloss-overrides with provenance)
  ...
  const synthesisOverrides =
    window.SYNTHESIS_GLOSS_OVERRIDES?.by_word_index || {};
  ...
  Object.entries(synthesisOverrides).forEach(([index, entry]) => {
    if (!entry?.gloss) return;
    preferredBaseGlosses.set(Number(index), entry.gloss);
    baseGlossProvenance.set(Number(index), {
      status: "overridden",
      signal: entry.source || "override",
      gloss: entry.gloss,
    });
  });
  ...
  const exactRoutesForCell = (cell) => {
    ...
    // antiquity rule keeps ordering them — they are 2026 rows,
  ...
  // V6.1 · antiquity ordering (rule v2 · 2026-08-10):
  // oldest source year first; sources after 1940 or without a
  ...
  // All D+M records carrying a route's text, oldest source first — one
  ...
  const renderWordShelf = ({
    ...
    shape.cells.forEach((cell) => {
      ...
      // V6.1 (rule v2 · 2026-08-10): pills sort by antiquity — oldest
      // source first, post-1940 and unyeared sources last —
  ...
  const createExactVerse = (verse, section) => {
    ...
    orderedWords.forEach((word) => {
      ...
      // derived draft is distinguishable from an overridden or user-chosen
  ...
  // and sorts last — the same lastuary shape the antiquity rule uses for
  ...
  const v5WitnessFamilies = (() => {
    ...
    [
      ...
      families.set(key, {
        ...
        // chronology the antiquity rule was always about — Rashi
  ...
  function publishAudit() {
    ...
    const audit = {
      ...
      synthesis_defaults_overridden: Object.keys(synthesisOverrides)
        .length,
  ...
  const v5PopulateWorkSelect = (select) => {
    ...
    [...v5WitnessFamilies.values()]
      ...
      // antiquity rule, applied to works.
      .sort((left, right) => {
