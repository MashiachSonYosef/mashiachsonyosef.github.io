...
function Append-LexicalHudScript {
  ...
  [void]$Builder.AppendLine('      };')
      ...
      [void]$Builder.AppendLine('      const sourceGroupsForStrict = (view, strictBuckets) => {')
      [void]$Builder.AppendLine('        const groups = [];')
      [void]$Builder.AppendLine('        const surfaceRenderings = cleanStrictRenderings(view.surface_renderings || []);')
      [void]$Builder.AppendLine('        if (surfaceRenderings.length) groups.push(makeSourceGroup(view.hebrew_word || view.surface_word || "Clicked form", surfaceRenderings, view.source_rows));')
      [void]$Builder.AppendLine('        strictEntriesForView(view).forEach((entry) => {')
      [void]$Builder.AppendLine('          const bucket = isAramaicEntry(entry) ? strictBuckets.aramaic : strictBuckets.hebrew;')
      ...
      [void]$Builder.AppendLine('          if (renderings.length) groups.push(makeSourceGroup(entry.lemma || entry.match_key || view.hebrew_word || view.surface_word, renderings, sourceRowsForEntry(view, entry)));')
      [void]$Builder.AppendLine('        });')
  ...
  [void]$Builder.AppendLine('        if (!potentialNode || !relatedNode) return;')
  [void]$Builder.AppendLine('        potentialNode.replaceChildren();')
  [void]$Builder.AppendLine('        relatedNode.replaceChildren();')
  [void]$Builder.AppendLine('        if (!hasLexicalEntry) { setRowHidden(root, "[data-hud-potential-row]", false); setRowHidden(root, "[data-hud-related-row]", true); potentialNode.textContent = "Context unresolved. No lexical entry yet."; return; }')
  [void]$Builder.AppendLine('        const entries = cleanValues(view.possible_entries);')
  ...
  [void]$Builder.AppendLine('        setRowHidden(root, "[data-hud-potential-row]", hasStrict ? !potentialEntries.length : false);')
  [void]$Builder.AppendLine('        setRowHidden(root, "[data-hud-related-row]", !relatedEntries.length);')
  [void]$Builder.AppendLine('        if (!potentialEntries.length) {')
  [void]$Builder.AppendLine('          if (!hasStrict) potentialNode.textContent = "Context unresolved.";')
  ...
  [void]$Builder.AppendLine('            details.appendChild(renderEntryList(potentialEntries, "Potential option"));')
      [void]$Builder.AppendLine('            appendSecondarySources(details, view, potentialEntries, "Sources / licenses for potential options");')
  ...
  [void]$Builder.AppendLine('            potentialNode.appendChild(renderEntryList(potentialEntries, "Potential option"));')
  [void]$Builder.AppendLine('          }')
  [void]$Builder.AppendLine('        }')
  [void]$Builder.AppendLine('        if (relatedEntries.length) {')
  [void]$Builder.AppendLine('          const details = document.createElement("details");')
  [void]$Builder.AppendLine('          details.className = "other-entries";')
  [void]$Builder.AppendLine('          const summary = document.createElement("summary");')
  [void]$Builder.AppendLine('          summary.textContent = "Show related options";')
  [void]$Builder.AppendLine('          details.appendChild(summary);')
  [void]$Builder.AppendLine('          details.appendChild(renderEntryList(relatedEntries, "Related option"));')
      [void]$Builder.AppendLine('          appendSecondarySources(details, view, relatedEntries, "Sources / licenses for related options");')
  ...
  [void]$Builder.AppendLine('          const hasStrict = strictBuckets.hebrew.length > 0 || strictBuckets.aramaic.length > 0;')
  [void]$Builder.AppendLine('          setRowHidden(hud, "[data-hud-hebrew-strict-row]", !strictBuckets.hebrew.length);')
  [void]$Builder.AppendLine('          setRowHidden(hud, "[data-hud-aramaic-strict-row]", !strictBuckets.aramaic.length);')
  [void]$Builder.AppendLine('          setList(hud, "[data-hud-hebrew-strict]", strictBuckets.hebrew);')
