...
function Get-HomeGroup {
  param([object]$Source)
  if ([string]$Source.work_id -eq 'orot') { return 'Thought / Musar / Chasidut' }
...
function Get-LibrarySubgroup {
  ...
  switch ($group) {
    ...
    'Thought / Musar / Chasidut' {
      $slugParts = @($Source.work_slug -split '[\\/]' | Where-Object { $_ })
      $firstSlug = if ($slugParts.Count -gt 0) { $slugParts[0] } else { '' }
      if ([string]$Source.work_id -eq 'orot') { return 'Modern Hebrew Thought' }
...
function Append-SiteHead {
  ...
  if ($IncludeLexicalStyles) {
    ...
    [void]$Builder.AppendLine('    .lexical-hud { position: fixed; inset: 0; z-index: 1000; background: rgba(10,14,18,0.82); padding: clamp(12px, 3vw, 28px); overflow: auto; }')
    [void]$Builder.AppendLine('    .lexical-hud[hidden] { display: none; }')
    [void]$Builder.AppendLine('    .hud-panel { width: min(980px, 100%); min-height: min(760px, calc(100vh - clamp(24px, 6vw, 56px))); margin: 0 auto; border: 1px solid var(--line); background: var(--panel-2); padding: clamp(16px, 3vw, 28px); box-shadow: 0 18px 70px rgba(0,0,0,0.5); }')
    [void]$Builder.AppendLine('    .hud-head { display: grid; grid-template-columns: 1fr auto; gap: 12px; align-items: start; margin-bottom: 18px; }')
    [void]$Builder.AppendLine('    .hud-clicked-word { min-width: 0; text-align: center; color: var(--hebrew); font-family: var(--hebrew-font); font-size: clamp(2rem, 7vw, 4.8rem); line-height: 1.08; overflow-wrap: anywhere; }')
    [void]$Builder.AppendLine('    .hud-close { border: 1px solid var(--line-2); background: transparent; color: var(--muted); width: 34px; height: 34px; line-height: 1; cursor: pointer; }')
    ...
    [void]$Builder.AppendLine('    .claim-row { border: 1px solid var(--line); background: rgba(255,255,255,0.025); padding: 12px; display: grid; gap: 6px; min-width: 0; overflow-wrap: anywhere; }')
    [void]$Builder.AppendLine('    .claim-row-head { display: grid; gap: 7px; justify-items: center; text-align: center; }')
    ...
    [void]$Builder.AppendLine('    .claim-hebrew { color: var(--hebrew); font-family: var(--hebrew-font); font-size: clamp(1.35rem, 3.8vw, 2.25rem); line-height: 1.12; }')
    [void]$Builder.AppendLine('    .claim-transliteration, .claim-source, .claim-license { color: var(--muted); font-size: 0.86rem; margin: 0; }')
    [void]$Builder.AppendLine('    .claim-renderings { color: var(--text); margin: 0; text-align: center; font-size: 1.02rem; }')
...
function Append-LexicalHudScript {
  ...
  [void]$Builder.AppendLine('      if (hud.parentElement !== document.body) document.body.appendChild(hud);')
      [void]$Builder.AppendLine('      const closeHud = () => {')
  ...
  [void]$Builder.AppendLine('        hud.scrollTop = 0;')
  [void]$Builder.AppendLine('      };')
  ...
  [void]$Builder.AppendLine('      };')
      ...
      [void]$Builder.AppendLine('        head.append(hebrew, badge);')
      ...
      [void]$Builder.AppendLine('          const lemmaRenderings = cleanStrictRenderings(entry.strict_renderings || []);')
      [void]$Builder.AppendLine('          if (!lemmaRenderings.length) return;')
      [void]$Builder.AppendLine('          claims.lemma.push(claimFromEntry(view, entry, "Strict Lemma", lemmaRenderings));')
      [void]$Builder.AppendLine('          const laneRenderings = lemmaRenderings.filter((rendering) => bucket.includes(rendering));')
      [void]$Builder.AppendLine('          if (!laneRenderings.length) return;')
      [void]$Builder.AppendLine('          if (surfaceRenderings.length && laneRenderings.every((rendering) => surfaceRenderings.includes(rendering))) return;')
      [void]$Builder.AppendLine('          claims[bucketName].push(claimFromEntry(view, entry, bucketName === "aramaic" ? "Strict Aramaic" : "Strict Hebrew", laneRenderings));')
      [void]$Builder.AppendLine('        });')
  ...
  [void]$Builder.AppendLine('        });')
  [void]$Builder.AppendLine('      });')
      [void]$Builder.AppendLine('      document.querySelectorAll("[data-hud-close]").forEach((button) => button.addEventListener("click", closeHud));')
  [void]$Builder.AppendLine('      hud.addEventListener("click", (event) => {')
  [void]$Builder.AppendLine('        closeHud();')
  [void]$Builder.AppendLine('      });')
  [void]$Builder.AppendLine('      document.addEventListener("keydown", (event) => { if (event.key === "Escape" && !hud.hidden) closeHud(); });')
...
foreach ($source in $renderSources) {
  ...
  if ($workHasLexical) {
    ...
    [void]$page.AppendLine('    <div class="hud-panel" role="dialog" aria-modal="true" aria-label="Lexical HUD">')
    [void]$page.AppendLine('    <div class="hud-head"><div class="hud-clicked-word" data-hud-word lang="he" dir="rtl">N/A</div><button class="hud-close" type="button" data-hud-close aria-label="Close lexical HUD">&times;</button></div>')
