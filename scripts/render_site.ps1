...
function Append-LexicalHudScript {
  ...
  [void]$Builder.AppendLine('      };')
      ...
      [void]$Builder.AppendLine('        if (status === "Unresolved") return { score: 0, reasons: ["no resolved lexical row"] };')
      ...
      [void]$Builder.AppendLine('        return { score: clampScore(score, floors[status] ?? 0, caps[status] ?? 99), reasons: uniqueValues(reasons) };')
      [void]$Builder.AppendLine('      };')
      ...
      [void]$Builder.AppendLine('        return { status: rowStatus, confidence: scored.score, confidence_reasons: scored.reasons, hebrew: normalizeHebrewDisplay(hebrew || "N/A"), transliteration: cleanTransliteration(transliteration), renderings: cleanRenderings, rows: sourceRows };')
      [void]$Builder.AppendLine('      };')
      ...
      [void]$Builder.AppendLine('          badge.textContent = `${claim.status || "Potential"} · ${Number.isFinite(claim.confidence) ? claim.confidence : 0}%`;')
      [void]$Builder.AppendLine('          const hebrew = document.createElement("strong");')
