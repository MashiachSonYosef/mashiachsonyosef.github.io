...
function Append-LexicalHudScript {
  ...
  [void]$Builder.AppendLine('        const strictEntries = [...likelyEntries, ...exactCleanEntries].filter((entry, index, entries) => entries.findIndex((candidate) => candidate.entry_key === entry.entry_key) === index);')
  ...
  [void]$Builder.AppendLine('        const likelyEntries = cleanValues(view.possible_entries).filter((entry) => entry.context_role === "likely_contextual");')
  [void]$Builder.AppendLine('        const exactCleanEntries = cleanValues(view.possible_entries).filter((entry) => entry.source_family !== "kaikki" && entry.source_family !== "wiktionary" && (normalizeHebrewKey(entry.match_key) === view.normalized_word || normalizeHebrewKey(entry.lemma) === view.normalized_word));')
  [void]$Builder.AppendLine('        return new Set([...likelyEntries, ...exactCleanEntries].map((entry) => entry.entry_key).filter(Boolean));')
  [void]$Builder.AppendLine('      };')
  ...
  [void]$Builder.AppendLine('      };')
      [void]$Builder.AppendLine('      const sourceRowsForEntries = (view, entries) => {')
      [void]$Builder.AppendLine('        const sourceKeys = new Set(cleanValues(entries).map((entry) => `${entry.source_family || ""}|${entry.source_id || ""}`));')
      [void]$Builder.AppendLine('        return cleanValues(view.secondary_source_rows).filter((row) => sourceKeys.has(`${row.source_family || ""}|${row.source_id || ""}`));')
      [void]$Builder.AppendLine('      };')
      [void]$Builder.AppendLine('      const appendSecondarySources = (container, view, entries, label) => {')
      [void]$Builder.AppendLine('        const rows = sourceRowsForEntries(view, entries);')
  ...
  [void]$Builder.AppendLine('        sourceDetails.append(sourceSummary, sourceBox);')
      [void]$Builder.AppendLine('        renderSources(sourceBox, rows);')
