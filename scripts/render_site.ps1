...
function Append-LexicalHudScript {
  ...
  [void]$Builder.AppendLine('      const getStrictRenderings = (view) => cleanValues((view.surface_renderings && view.surface_renderings.length) ? view.surface_renderings : view.strict_renderings);')
  [void]$Builder.AppendLine('      const isAramaicView = (view) => {')
  [void]$Builder.AppendLine('        const haystack = [view.match_method, view.source_name, view.context_note, view.surface_context_note, view.lexical_language, view.language, view.language_code].filter(Boolean).join(" ").toLowerCase();')
  ...
  [void]$Builder.AppendLine('        return cleanValues(view.possible_entries).some((entry) => [entry.source_name, entry.relation_label, entry.language, entry.language_code].filter(Boolean).join(" ").toLowerCase().includes("aramaic"));')
  [void]$Builder.AppendLine('      };')
  ...
  [void]$Builder.AppendLine('        if (!node) return;')
  [void]$Builder.AppendLine('        node.replaceChildren();')
  [void]$Builder.AppendLine('        if (label) label.textContent = "Possible entries";')
  [void]$Builder.AppendLine('        const strictValues = getStrictRenderings(view);')
  [void]$Builder.AppendLine('        const hasStrict = strictValues.length > 0;')
  [void]$Builder.AppendLine('        if (!hasLexicalEntry) { setRowHidden(root, "[data-hud-renderings-row]", false); node.textContent = "No lexical entry yet."; return; }')
  ...
  [void]$Builder.AppendLine('        const likelyEntries = hasStrict ? [] : entries.filter((entry) => entry.context_role === "likely_contextual");')
  [void]$Builder.AppendLine('        const otherEntries = hasStrict ? entries.filter((entry) => entry.context_role !== "likely_contextual") : entries.filter((entry) => entry.context_role !== "likely_contextual");')
  [void]$Builder.AppendLine('        if (!likelyEntries.length && !otherEntries.length) { setRowHidden(root, "[data-hud-renderings-row]", hasStrict); if (!hasStrict) node.textContent = "Context not resolved."; return; }')
  [void]$Builder.AppendLine('        setRowHidden(root, "[data-hud-renderings-row]", false);')
  [void]$Builder.AppendLine('        const note = document.createElement("p");')
  [void]$Builder.AppendLine('        note.className = "lexical-context-note";')
  [void]$Builder.AppendLine('        note.textContent = likelyEntries.length ? (view.surface_context_note || view.context_note || "Context resolved.") : (hasStrict ? "Alternate / caution entries." : "Context not resolved.");')
  [void]$Builder.AppendLine('        if (!hasStrict || likelyEntries.length) node.appendChild(note);')
  ...
  [void]$Builder.AppendLine('          const strictValues = hasLexicalEntry ? getStrictRenderings(view) : [];')
  [void]$Builder.AppendLine('          const isAramaic = hasLexicalEntry && isAramaicView(view);')
  [void]$Builder.AppendLine('          setRowHidden(hud, "[data-hud-hebrew-strict-row]", isAramaic || !strictValues.length);')
  [void]$Builder.AppendLine('          setRowHidden(hud, "[data-hud-aramaic-strict-row]", !isAramaic || !strictValues.length);')
  [void]$Builder.AppendLine('          setList(hud, isAramaic ? "[data-hud-aramaic-strict]" : "[data-hud-hebrew-strict]", strictValues);')
...
foreach ($source in $renderSources) {
  ...
  if ($workHasLexical) {
    ...
    [void]$page.AppendLine('      <div class="lexical-field-row" data-hud-hebrew-strict-row><dt>Hebrew strict</dt><dd data-hud-hebrew-strict>N/A</dd></div>')
    [void]$page.AppendLine('      <div class="lexical-field-row" data-hud-aramaic-strict-row hidden><dt>Aramaic strict</dt><dd data-hud-aramaic-strict>N/A</dd></div>')
    [void]$page.AppendLine('      <div class="lexical-field-row" data-hud-breakdown-row><dt>Breakdown</dt><dd data-hud-breakdown>N/A</dd></div>')
    [void]$page.AppendLine('      <div class="lexical-field-row" data-hud-renderings-row><dt data-hud-renderings-label>Possible entries</dt><dd data-hud-renderings>N/A</dd></div>')
