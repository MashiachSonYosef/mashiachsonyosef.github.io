...
function Append-SiteHead {
  ...
  if ($IncludeLexicalStyles) {
    ...
    [void]$Builder.AppendLine('    .claim-row { border: 1px solid var(--line); background: rgba(255,255,255,0.025); padding: 10px; display: grid; gap: 5px; }')
...
function Append-LexicalHudScript {
  ...
  [void]$Builder.AppendLine('      };')
      ...
      [void]$Builder.AppendLine('        });')
      [void]$Builder.AppendLine('      };')
      [void]$Builder.AppendLine('      const makeClaim = (status, hebrew, renderings, rows, transliteration = "") => {')
      ...
      [void]$Builder.AppendLine('        return { status: rowStatus, hebrew: normalizeHebrewDisplay(hebrew || "N/A"), transliteration: cleanTransliteration(transliteration), renderings: cleanStrictRenderings(renderings || []), rows: sourceRows };')
      [void]$Builder.AppendLine('      };')
      ...
      [void]$Builder.AppendLine('      const claimFromEntry = (view, entry, status, renderings = null) => makeClaim(status, entryHebrewLabel(entry, view), renderings || entry.strict_renderings || [], sourceRowsForEntry(view, entry), entry.transliteration || entry.surface_transliteration || "");')
      [void]$Builder.AppendLine('      const sourceSummary = (rows) => {')
      [void]$Builder.AppendLine('        const row = cleanValues(rows)[0];')
      [void]$Builder.AppendLine('        if (!row) return { source: "Source metadata incomplete", license: "N/A" };')
      [void]$Builder.AppendLine('        const sourceId = row.source_id ? ` ${row.source_id}` : "";')
      [void]$Builder.AppendLine('        return { source: `${row.source_name || "Source"}${sourceId}`, license: row.license || "N/A" };')
      [void]$Builder.AppendLine('      };')
      [void]$Builder.AppendLine('      const appendClaimRows = (node, claims) => {')
      [void]$Builder.AppendLine('        if (!node) return;')
      [void]$Builder.AppendLine('        node.replaceChildren();')
      [void]$Builder.AppendLine('        const rows = uniqueClaims(claims);')
      [void]$Builder.AppendLine('        if (!rows.length) { node.textContent = "N/A"; return; }')
      [void]$Builder.AppendLine('        const list = document.createElement("div");')
      [void]$Builder.AppendLine('        list.className = "claim-row-list";')
      [void]$Builder.AppendLine('        rows.forEach((claim) => {')
      [void]$Builder.AppendLine('          const card = document.createElement("section");')
      ...
      [void]$Builder.AppendLine('          badge.textContent = claim.status || "Potential";')
      [void]$Builder.AppendLine('          const hebrew = document.createElement("strong");')
      ...
      [void]$Builder.AppendLine('          const claim = makeClaim(status, view.hebrew_word || view.surface_word || "Clicked form", surfaceRenderings, view.source_rows, view.surface_transliteration || view.transliteration || "");')
      ...
      [void]$Builder.AppendLine('          if (strictBuckets.hebrew.length) claims.hebrew.push(makeClaim("Strict Hebrew", view.hebrew_word || view.surface_word || "Clicked form", strictBuckets.hebrew, view.source_rows, view.surface_transliteration || view.transliteration || ""));')
      [void]$Builder.AppendLine('          if (strictBuckets.aramaic.length) claims.aramaic.push(makeClaim("Strict Aramaic", view.hebrew_word || view.surface_word || "Clicked form", strictBuckets.aramaic, view.source_rows, view.surface_transliteration || view.transliteration || ""));')
      [void]$Builder.AppendLine('        }')
      [void]$Builder.AppendLine('        return { hebrew: uniqueClaims(claims.hebrew), aramaic: uniqueClaims(claims.aramaic) };')
      [void]$Builder.AppendLine('      };')
      [void]$Builder.AppendLine('      const makeSourceGroup = (label, renderings, rows) => ({ label: normalizeHebrewDisplay(label || "Lexical claim"), renderings: cleanStrictRenderings(renderings || []), rows: cleanValues(rows) });')
      ...
      [void]$Builder.AppendLine('      const sourceGroupsForClaims = (claims) => uniqueSourceGroups(cleanValues(claims).map((claim) => makeSourceGroup(claim.hebrew, claim.renderings, claim.rows)));')
      ...
      [void]$Builder.AppendLine('      const sourceGroupsForVisible = (view, strictClaims, hasStrict) => {')
      [void]$Builder.AppendLine('        const strictGroups = sourceGroupsForStrict(strictClaims);')
  ...
  [void]$Builder.AppendLine('        if (!potentialNode || !relatedNode || !cautionNode) return;')
  ...
  [void]$Builder.AppendLine('        if (!hasLexicalEntry) { setRowHidden(root, "[data-hud-potential-row]", false); setRowHidden(root, "[data-hud-related-row]", true); setRowHidden(root, "[data-hud-caution-row]", true); appendClaimRows(potentialNode, [makeClaim("Unresolved", view.hebrew_word || view.surface_word || "Clicked form", ["No lexical entry yet."], [])]); return; }')
  [void]$Builder.AppendLine('        const entries = cleanValues(view.possible_entries);')
  ...
  [void]$Builder.AppendLine('        const cautionClaims = uniqueClaims([...potentialClaims, ...relatedClaims].filter((claim) => claim.status === "Caution"));')
  ...
  [void]$Builder.AppendLine('        setRowHidden(root, "[data-hud-potential-row]", hasStrict ? !visiblePotentialClaims.length : false);')
  [void]$Builder.AppendLine('        setRowHidden(root, "[data-hud-related-row]", !visibleRelatedClaims.length);')
  [void]$Builder.AppendLine('        setRowHidden(root, "[data-hud-caution-row]", !cautionClaims.length);')
  [void]$Builder.AppendLine('        if (!visiblePotentialClaims.length) {')
  [void]$Builder.AppendLine('          if (!hasStrict) appendClaimRows(potentialNode, [makeClaim("Unresolved", view.hebrew_word || view.surface_word || "Clicked form", ["Context unresolved."], [])]);')
  ...
  [void]$Builder.AppendLine('          if (hasStrict) {')
  [void]$Builder.AppendLine('            const summary = document.createElement("summary");')
  [void]$Builder.AppendLine('            summary.textContent = "Show potential options";')
  [void]$Builder.AppendLine('            details.appendChild(summary);')
  [void]$Builder.AppendLine('            const claimBox = document.createElement("div");')
  [void]$Builder.AppendLine('            appendClaimRows(claimBox, visiblePotentialClaims);')
  [void]$Builder.AppendLine('            details.appendChild(claimBox);')
      [void]$Builder.AppendLine('            const sourceDetails = document.createElement("details");')
      [void]$Builder.AppendLine('            sourceDetails.className = "source-details";')
      [void]$Builder.AppendLine('            const sourceSummary = document.createElement("summary");')
      [void]$Builder.AppendLine('            sourceSummary.textContent = "Sources / licenses for potential options";')
      [void]$Builder.AppendLine('            const sourceBox = document.createElement("div");')
      [void]$Builder.AppendLine('            sourceDetails.append(sourceSummary, sourceBox);')
      [void]$Builder.AppendLine('            renderSourceGroups(sourceBox, sourceGroupsForClaims(visiblePotentialClaims));')
      [void]$Builder.AppendLine('            details.appendChild(sourceDetails);')
  [void]$Builder.AppendLine('            potentialNode.appendChild(details);')
  [void]$Builder.AppendLine('          } else {')
  [void]$Builder.AppendLine('            const note = document.createElement("p");')
  [void]$Builder.AppendLine('            note.className = "lexical-context-note";')
  [void]$Builder.AppendLine('            note.textContent = "Context unresolved.";')
  [void]$Builder.AppendLine('            potentialNode.appendChild(note);')
  [void]$Builder.AppendLine('            appendClaimRows(potentialNode, visiblePotentialClaims);')
  [void]$Builder.AppendLine('          }')
  [void]$Builder.AppendLine('        }')
  [void]$Builder.AppendLine('        if (visibleRelatedClaims.length) {')
  [void]$Builder.AppendLine('          const details = document.createElement("details");')
  [void]$Builder.AppendLine('          details.className = "other-entries";')
  [void]$Builder.AppendLine('          const summary = document.createElement("summary");')
  [void]$Builder.AppendLine('          summary.textContent = "Show related options";')
  [void]$Builder.AppendLine('          details.appendChild(summary);')
  [void]$Builder.AppendLine('          const claimBox = document.createElement("div");')
  [void]$Builder.AppendLine('          appendClaimRows(claimBox, visibleRelatedClaims);')
  [void]$Builder.AppendLine('          details.appendChild(claimBox);')
      [void]$Builder.AppendLine('          const sourceDetails = document.createElement("details");')
      [void]$Builder.AppendLine('          sourceDetails.className = "source-details";')
      [void]$Builder.AppendLine('          const sourceSummary = document.createElement("summary");')
      [void]$Builder.AppendLine('          sourceSummary.textContent = "Sources / licenses for related options";')
      [void]$Builder.AppendLine('          const sourceBox = document.createElement("div");')
      [void]$Builder.AppendLine('          sourceDetails.append(sourceSummary, sourceBox);')
      [void]$Builder.AppendLine('          renderSourceGroups(sourceBox, sourceGroupsForClaims(visibleRelatedClaims));')
      [void]$Builder.AppendLine('          details.appendChild(sourceDetails);')
  [void]$Builder.AppendLine('          relatedNode.appendChild(details);')
  [void]$Builder.AppendLine('        }')
  [void]$Builder.AppendLine('        if (cautionClaims.length) appendClaimRows(cautionNode, cautionClaims);')
  [void]$Builder.AppendLine('      };')
  ...
  [void]$Builder.AppendLine('          renderSourceGroups(hud.querySelector("[data-hud-sources]"), sourceGroupsForVisible(view, strictClaims, hasStrict));')
