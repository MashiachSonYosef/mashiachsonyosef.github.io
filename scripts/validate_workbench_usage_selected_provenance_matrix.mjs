...
const artifactPath = cleanRelativePath(process.argv[2] || '.local-cache/workbench-evidence/usage-selected-provenance-matrix.json');
...
if (artifact.artifact_type !== 'workbench_usage_selected_provenance_matrix') {
  issues.push('artifact_type must be workbench_usage_selected_provenance_matrix');
}
if (!String(artifact.policy || '').includes('Audit-only selected provenance matrix')) {
  issues.push('policy must identify audit-only selected provenance matrix');
}
...
const provenanceRows = Array.isArray(artifact.provenance_rows) ? artifact.provenance_rows : [];
validateCounts(provenanceRows);
for (const [index, row] of provenanceRows.entries()) validateProvenanceRow(`provenance_rows[${index}]`, row);
...
if (issues.length) {
  console.error(`Workbench usage selected provenance matrix validation failed with ${issues.length} issue(s):`);
  for (const issue of issues) console.error(`- ${issue}`);
  process.exit(1);
}

console.log(`Validated usage selected provenance matrix ${artifactPath}: buckets ${provenanceRows.length}; selected rows ${artifact.counts.selected_rows}`);

function validateCounts(rows) {
  if (Number(artifact.counts?.provenance_buckets || 0) !== rows.length) issues.push('provenance_buckets count must equal provenance_rows length');
  ...
  if (Number(artifact.counts?.rows_with_license_metadata || 0) !== Number(artifact.counts?.selected_rows || 0)) {
    issues.push('rows_with_license_metadata must equal selected_rows');
  }
  if (Number(artifact.counts?.rows_with_version_metadata || 0) !== Number(artifact.counts?.selected_rows || 0)) {
    issues.push('rows_with_version_metadata must equal selected_rows');
  }
  if (Number(artifact.counts?.missing_or_unrecognized_license_rows || 0) !== 0) {
    issues.push('missing_or_unrecognized_license_rows must be 0');
  }
  ...
  if (Number(artifact.counts?.selected_rows || 0) !== selectedRows) issues.push('selected_rows must match provenance row totals');
  if (Number(artifact.counts?.sample_occurrences || 0) !== sampleRows) issues.push('sample_occurrences must match sample totals');
...
function validateProvenanceRow(context, row) {
  requireFields(row, [
    'provenance_id',
    'license',
    'license_url',
  ...
  if (row.provenance_flags?.observed_usage_only !== true) issues.push(`${context}: provenance_flags.observed_usage_only must be true`);
  if (row.provenance_flags?.reader_facing !== false) issues.push(`${context}: provenance_flags.reader_facing must be false`);
  if (row.provenance_flags?.audit_only !== true) issues.push(`${context}: provenance_flags.audit_only must be true`);
  if (row.provenance_flags?.license_metadata_present !== true) issues.push(`${context}: provenance_flags.license_metadata_present must be true`);
  if (row.provenance_flags?.version_metadata_present !== true) issues.push(`${context}: provenance_flags.version_metadata_present must be true`);
  if (row.provenance_flags?.license_recognized_for_selected_lane !== true) {
    issues.push(`${context}: provenance_flags.license_recognized_for_selected_lane must be true`);
  }
...
function validateSample(context, sample, row) {
  ...
  if (sample.license !== row.license) issues.push(`${context}: sample license must match provenance row`);
  if (sample.license_url !== row.license_url) issues.push(`${context}: sample license_url must match provenance row`);
  if (sample.version_title !== row.version_title) issues.push(`${context}: sample version_title must match provenance row`);
  if (sample.version_source !== row.version_source) issues.push(`${context}: sample version_source must match provenance row`);
