...
const included = targets.filter((target) => target.include_in_next_expansion === true);

if (data.schema_version !== 1) issues.push('schema_version must be 1');
if (data.artifact_type !== 'reader_workbench_expansion_targets') issues.push('artifact_type must be reader_workbench_expansion_targets');
...
if (!included.length) issues.push('targets must include at least one expansion target');
...
for (const [index, target] of included.entries()) {
  ...
  if (target.source_audit_scope !== 'tracked_source') {
    issues.push(`${context}: source_audit_scope must be tracked_source`);
  }
...
for (const lane of ['tanakh_base', 'tanakh_commentary', 'halakhah_commentary', 'aramaic_targum']) {
  if (!lanes.has(lane)) warnings.push(`representative lane not present in included targets: ${lane}`);
}

for (const [index, target] of (data.deferred_targets || []).entries()) {
  const context = `deferred_targets[${index}] ${target.work_id || 'missing-work-id'}`;
  if (!target.reason) issues.push(`${context}: missing reason`);
  if (target.include_in_next_expansion === true) issues.push(`${context}: deferred target must not be included`);
}

if (issues.length) {
  console.error(`Reader Workbench expansion target validation failed with ${issues.length} issue(s):`);
...
console.log('Reader Workbench expansion target validation passed.');
console.log(JSON.stringify({
  included_targets: included.length,
  lanes: [...lanes].sort(),
  deferred_targets: (data.deferred_targets || []).length,
  warnings,
}, null, 2));
