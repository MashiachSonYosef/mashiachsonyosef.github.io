...
function makeKaikkiDefinitionClaim(entry, lineNumber) {
  ...
  return {
    ...
    match_type: 'lemma',
    confidence: 94,
    answer_score: Math.max(0, Math.min(100, 94 + meaningRank.answer_score_adjustment)),
  ...
  for (const layer of manifest.layer_files || []) {
    ...
    for (const entry of data.entries || []) {
      ...
      claims.push({
        ...
        answer_score: Math.max(0, Math.min(100, (layer.source_family === 'workspace' ? 95 : 90) + meaningRank.answer_score_adjustment)),
        source_order: order,
        meaning_quality: meaningRank.meaning_quality,
        answer_eligible: meaningRank.answer_eligible,
        part_of_speech: '',
  ...
  const manifest = {
    ...
    route_policy: {
      ...
      hud_copy_rules: [
        ...
        'Keep source and license rows visible by default.',
        'Use horizontal card lanes when a route family has many real matches.',
      ],
    },
