...
const options = {
  ...
  candidatePrep: 'reports/agent10-candidate-page-4-shipment-prep-2026-06-02.md',
  agent4Proof: 'reports/agent4-leviticus-live-browser-click-proof-2026-06-02.json',
  agent4ProofReport: 'reports/agent4-leviticus-live-browser-click-proof-2026-06-02.md',
  agent4Screenshot: 'reports/agent4-leviticus-live-browser-click-proof-2026-06-02.png',
  liveGuard: `reports/agent10-live-public-old-hud-guard-${dateSlug}-post-orot-zero-safe-pilot-docket.json`,
  jsonReport: `reports/agent10-agent6-ready-leviticus-runtime-review-docket-${dateSlug}.json`,
  report: `reports/agent10-agent6-ready-leviticus-runtime-review-docket-${dateSlug}.md`,
...
const liveLane = (releaseTrain.live_lane_checks || []).find((row) => row.work_id === 'leviticus') || {};
const lane = (releaseTrain.lanes || []).find((row) => row.lane_id === 'leviticus_agent6_runtime_review') || {};
...
if (lane.current_state !== 'live_current_hud_package_and_agent4_proof_exist_no_agent6_verdict') issues.push('Leviticus lane current_state is not the expected no-Agent-6-verdict state.');
if (liveLane.page_status !== 200) issues.push('Leviticus live page does not report 200 in release train.');
if ((liveLane.page_hard_old_marker_hits || []).length !== 0) issues.push('Leviticus live lane reports hard old-HUD marker hits.');
if (liveLane.manifest_status !== 200 || liveLane.reader_hints_status !== 200 || liveLane.route_lookup_manifest_status !== 200) issues.push('Leviticus public-HUD dependencies are not all 200.');
if (liveLane.hint_count !== 3869) issues.push('Leviticus hint count drifted from expected 3869.');
if (liveLane.route_key_count !== 1909) issues.push('Leviticus route key count drifted from expected 1909.');
if (liveLane.shard_count !== 1137) issues.push('Leviticus shard count drifted from expected 1137.');
if (liveLane.card_count !== 5237) issues.push('Leviticus card count drifted from expected 5237.');

if (agent4Proof.artifact_type !== 'agent4_leviticus_live_browser_click_proof') issues.push('Agent 4 proof artifact type is unexpected.');
...
const output = {
  schema_version: 1,
  artifact_type: 'agent10_agent6_ready_leviticus_runtime_review_docket',
  generated_at: generatedAt,
  generator: 'scripts/build_agent10_leviticus_runtime_review_docket.mjs',
  commit_basis: {
  ...
  boundary: {
    status: issues.length ? 'blocked_leviticus_runtime_review_docket' : 'agent6_ready_leviticus_runtime_review_docket_not_accepted',
    evidence_only: true,
    pipeline_only: true,
  ...
  review_request: {
    ...
    requested_verdict_type: 'pass_warn_block_on_leviticus_runtime_evidence_only',
    review_target: 'Leviticus exact public reader surface runtime evidence for candidate surface #4',
    ...
    specific_questions: [
      'Does the existing Agent 4 Leviticus browser proof satisfy the exact-surface runtime evidence boundary for Agent 6 review?',
      ...
      'Is the stale candidate-prep public-404 observation superseded by later bounded live proof for review-input purposes only?',
      'What exact blocker remains before Leviticus may be counted as a validated public reader surface under an Agent 6 boundary?',
    ],
  },
  summary: {
    status: issues.length ? 'blocked_leviticus_runtime_review_docket' : (warnings.length ? 'warn_agent6_ready_leviticus_runtime_review_docket_not_accepted' : 'agent6_ready_leviticus_runtime_review_docket_not_accepted'),
...
console.log(`Leviticus runtime review docket complete (${output.summary.status}). Report: ${options.report}`);
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--release-train') parsed.releaseTrain = cleanRelativePath(argv[++index]);
    else if (arg === '--candidate-prep') parsed.candidatePrep = cleanRelativePath(argv[++index]);
    else if (arg === '--agent4-proof') parsed.agent4Proof = cleanRelativePath(argv[++index]);
    else if (arg === '--agent4-proof-report') parsed.agent4ProofReport = cleanRelativePath(argv[++index]);
    else if (arg === '--agent4-screenshot') parsed.agent4Screenshot = cleanRelativePath(argv[++index]);
...
function writeReport(reportPath, data) {
  const lines = [
    ...
    '',
    ...data.stale_context_preserved.map((entry) => `- ${entry}`),
    '',
    '## Allowed Next Routes',
    '',
