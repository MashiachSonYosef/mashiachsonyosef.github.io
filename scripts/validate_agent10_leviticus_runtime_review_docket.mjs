...
const report = process.argv[2] || 'reports/agent10-agent6-ready-leviticus-runtime-review-docket-2026-06-03.json';
const data = readJson(report);
const issues = [];

expect(data.artifact_type === 'agent10_agent6_ready_leviticus_runtime_review_docket', 'unexpected artifact_type');
expect(data.boundary?.status === 'agent6_ready_leviticus_runtime_review_docket_not_accepted', 'unexpected boundary status');
...
expect(data.summary?.status === 'warn_agent6_ready_leviticus_runtime_review_docket_not_accepted', 'unexpected summary status');
expect(data.summary?.page === 'tanakh/leviticus/', 'unexpected page');
expect(data.summary?.work_id === 'leviticus', 'unexpected work_id');
expect(data.summary?.live_page_status === 200, 'Leviticus live page must be 200');
expect(data.summary?.live_page_hard_old_marker_hits === 0, 'Leviticus hard old-HUD hits must be 0');
...
expect(data.summary?.hint_count === 3869, 'hint count drifted');
expect(data.summary?.route_key_count === 1909, 'route key count drifted');
expect(data.summary?.shard_count === 1137, 'shard count drifted');
expect(data.summary?.card_count === 5237, 'card count drifted');
expect(data.summary?.max_shard_bytes === 49788, 'max shard bytes drifted');
...
expect(data.summary?.agent4_route_cards_clicked === 47, 'Agent 4 route cards clicked drifted');
expect(data.summary?.agent4_answer_cards_clicked === 2, 'Agent 4 answer cards clicked drifted');
expect(data.summary?.agent4_source_rows_clicked === 2, 'Agent 4 source rows clicked drifted');
...
if (issues.length) {
  console.error(`Agent 10 Leviticus runtime review docket validation failed with ${issues.length} issue(s):`);
  for (const issue of issues) console.error(`- ${issue}`);
  process.exit(1);
}

console.log(`Agent 10 Leviticus runtime review docket validation passed for ${report}.`);
