...
const artifactPath = cleanRelativePath(process.argv[2] || '.local-cache/workbench-evidence/usage-selected-work-hub-index.json');
...
if (artifact.artifact_type !== 'workbench_usage_selected_work_hub_index') {
  issues.push('artifact_type must be workbench_usage_selected_work_hub_index');
}
...
const rows = Array.isArray(artifact.work_hub_rows) ? artifact.work_hub_rows : [];
if (!rows.length) issues.push('work_hub_rows must be non-empty');
validateCounts(rows);
validateRows(rows);
...
if (issues.length) {
  console.error(`Selected work hub index validation failed with ${issues.length} issue(s):`);
  for (const issue of issues) console.error(`- ${issue}`);
  process.exit(1);
}

console.log(`Validated selected work hub index ${artifactPath}: hubs ${artifact.counts.hubs}; occurrence rows ${artifact.counts.occurrence_rows}; target links ${artifact.counts.target_links}`);
...
function validateCounts(rows) {
  for (const field of [
    ...
    'rows_with_source_links',
    'rows_with_work_anchors',
    'rows_with_marked_context',
    'rows_with_provenance',
    'target_work_samples',
    'target_work_samples_with_links',
    'target_work_samples_with_context',
  ...
  if (Number(artifact.counts?.hubs || 0) !== rows.length) issues.push('counts.hubs must equal work_hub_rows length');
  if (Number(artifact.counts?.hubs || 0) !== Number(artifact.counts?.expected_works || 0)) issues.push('hubs must equal expected_works');
  ...
  if (Number(artifact.counts?.source_refs || 0) !== Number(artifact.counts?.expected_source_refs || 0)) {
    issues.push('source_refs must equal expected_source_refs');
  }
  ...
  if (Number(artifact.counts?.rows_with_source_refs || 0) !== rows.length) issues.push('rows_with_source_refs must equal hubs');
  if (Number(artifact.counts?.rows_with_source_links || 0) !== rows.length) issues.push('rows_with_source_links must equal hubs');
  if (Number(artifact.counts?.rows_with_work_anchors || 0) !== rows.length) issues.push('rows_with_work_anchors must equal hubs');
  ...
  if (Number(artifact.counts?.target_work_samples_with_links || 0) !== Number(artifact.counts?.target_work_samples || 0)) {
    issues.push('target_work_samples_with_links must equal target_work_samples');
  }
  if (Number(artifact.counts?.target_work_samples_with_context || 0) !== Number(artifact.counts?.target_work_samples || 0)) {
    issues.push('target_work_samples_with_context must equal target_work_samples');
  }
...
function validateRows(rows) {
  ...
  for (const [index, row] of rows.entries()) {
    const prefix = `work_hub_rows[${index}]`;
    if (!row.work_hub_id) issues.push(`${prefix}.work_hub_id is required`);
    if (ids.has(row.work_hub_id)) issues.push(`${prefix}.work_hub_id is duplicated`);
    ids.add(row.work_hub_id);
    if (!row.work_slug) issues.push(`${prefix}.work_slug is required`);
    if (!row.work_title) issues.push(`${prefix}.work_title is required`);
    if (!Array.isArray(row.source_refs) || !row.source_refs.length) issues.push(`${prefix}.source_refs must be non-empty`);
    if (!Array.isArray(row.source_hrefs) || row.source_hrefs.length !== row.source_refs.length) {
      issues.push(`${prefix}.source_hrefs must match source_refs`);
    }
    if (!Array.isArray(row.work_anchor_hrefs) || row.work_anchor_hrefs.length !== row.source_refs.length) {
      issues.push(`${prefix}.work_anchor_hrefs must match source_refs`);
    }
    ...
    if (row.navigation_flags?.has_source_refs !== true) issues.push(`${prefix}.navigation_flags.has_source_refs must be true`);
    if (row.navigation_flags?.has_source_links !== true) issues.push(`${prefix}.navigation_flags.has_source_links must be true`);
    if (row.navigation_flags?.has_work_anchors !== true) issues.push(`${prefix}.navigation_flags.has_work_anchors must be true`);
    ...
    if (row.navigation_flags?.target_work_samples_have_links !== true) issues.push(`${prefix}.navigation_flags.target_work_samples_have_links must be true`);
    if (row.navigation_flags?.target_work_samples_have_context !== true) issues.push(`${prefix}.navigation_flags.target_work_samples_have_context must be true`);
  }
}
