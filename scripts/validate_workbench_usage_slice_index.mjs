...
const sliceIndexPath = cleanRelativePath(process.argv[2] || '.local-cache/workbench-evidence/usage-slice-index.json');
const artifact = readJson(sliceIndexPath);
const issues = [];
...
if (artifact.artifact_type !== 'workbench_usage_navigation_slice_index') {
  issues.push('artifact_type must be workbench_usage_navigation_slice_index');
}
if (!String(artifact.policy || '').includes('usage-navigation')) issues.push('policy must identify usage-navigation');
if (!String(artifact.filter?.slice_id || '').trim()) issues.push('filter.slice_id must be present');
if (!artifact.filter?.work_prefix && !artifact.filter?.source_ref_prefix) {
  issues.push('filter must include work_prefix or source_ref_prefix');
}
...
validateSamples();
walkNoForbiddenFields(artifact, sliceIndexPath);

if (issues.length) {
  console.error(`Workbench usage slice index validation failed with ${issues.length} issue(s):`);
  for (const issue of issues) console.error(`- ${issue}`);
  process.exit(1);
}

console.log(`Workbench usage slice index validation passed. Slice rows: ${artifact.counts.slice_rows}. Works: ${artifact.counts.works}.`);

function validateCounts() {
  for (const field of ['concordance_rows', 'slice_rows', 'works', 'clusters', 'route_ids']) {
  ...
  if (statusTotal !== Number(artifact.counts?.slice_rows || 0)) issues.push('counts.status_counts must sum to counts.slice_rows');
  ...
  if (routeStateTotal !== Number(artifact.counts?.slice_rows || 0)) {
    issues.push('counts.route_link_state_counts must sum to counts.slice_rows');
  }
...
function validateCollections() {
  ...
  else if (Number(artifact.counts?.slice_rows || 0) !== artifact.occurrence_refs.length) {
    issues.push(`counts.slice_rows expected ${artifact.occurrence_refs.length}, found ${artifact.counts?.slice_rows}`);
  }
}
...
function validateOccurrence(occurrence, context) {
  ...
  validateFilter(occurrence, context);
  if (!Array.isArray(occurrence.route_ids)) issues.push(`${context}.route_ids must be an array`);
}

function validateFilter(occurrence, context) {
  if (artifact.filter?.work_prefix && !String(occurrence.work_slug || '').startsWith(artifact.filter.work_prefix)) {
    issues.push(`${context}.work_slug does not match filter.work_prefix`);
  }
  if (artifact.filter?.source_ref_prefix && !String(occurrence.source_ref || '').startsWith(artifact.filter.source_ref_prefix)) {
    issues.push(`${context}.source_ref does not match filter.source_ref_prefix`);
  }
}
...
function validateIndexTotals(field, entries = []) {
  let rows = 0;
  for (const [index, entry] of entries.entries()) {
    rows += validateIndexEntry(entry, `${field}[${index}]`);
  }
  if (field !== 'routes' && rows !== Number(artifact.counts?.slice_rows || 0)) {
    issues.push(`${field} counts expected ${artifact.counts?.slice_rows}, found ${rows}`);
  }
}

function validateIndexEntry(entry, context) {
  if (!String(entry.key || '').trim()) issues.push(`${context}.key must be present`);
  if (!String(entry.label || '').trim()) issues.push(`${context}.label must be present`);
  ...
  if (!Array.isArray(entry.occurrence_ids)) issues.push(`${context}.occurrence_ids must be an array`);
  else if (entry.occurrence_ids.length !== rows) issues.push(`${context}.occurrence_ids length must equal rows`);
  return rows;
}

function validateSamples() {
  if (!Array.isArray(artifact.samples)) issues.push('samples must be an array');
  for (const [index, sample] of (artifact.samples || []).entries()) {
    for (const field of ['occurrence_id', 'candidate_id', 'cluster_id', 'status', 'source_ref', 'source_href', 'work_anchor_href', 'context_focus_marked']) {
      if (!String(sample?.[field] || '').trim()) issues.push(`samples[${index}].${field} must be present`);
    }
    if (!allowedStatuses.has(sample.status)) issues.push(`samples[${index}].status must be supported/candidate/weak`);
    validateWorkAnchor(sample.work_anchor_href, `samples[${index}]`);
    if (!Array.isArray(sample.route_ids)) issues.push(`samples[${index}].route_ids must be an array`);
  }
}
