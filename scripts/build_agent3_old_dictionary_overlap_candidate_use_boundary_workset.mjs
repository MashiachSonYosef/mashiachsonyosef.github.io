...
const defaults = {
  ...
  output: 'reports/agent3-old-dictionary-overlap-candidate-use-boundary-workset-2026-06-06.json',
...
const worksetRows = (boundaryTriageNavigation.candidate_rows || [])
  ...
  .sort(
    (left, right) =>
      triageRank(left.triage_group) - triageRank(right.triage_group) ||
      right.occurrences - left.occurrences ||
      left.queue_id.localeCompare(right.queue_id),
  );

const blockerRows = buildBlockerRows(worksetRows);
const triageGroupRows = buildTriageGroupRows(worksetRows);
const sourceFamilyRows = buildSourceFamilyRows(worksetRows);
const sourceFamilySetRows = buildSourceFamilySetRows(worksetRows);
const bucketSourceFamilySetRows = buildBucketSourceFamilySetRows(worksetRows);
...
const rowsWithNcOverlap = worksetRows.filter((row) =>
  row.classification_lanes.includes('noncommercial_educational_candidate'),
);
const rowsWithBlockedOverlap = worksetRows.filter((row) =>
  row.classification_lanes.includes('blocked_or_needs_review'),
);
const rowsWithTripleOverlap = worksetRows.filter(
  (row) =>
    row.classification_lanes.includes('noncommercial_educational_candidate') &&
    row.classification_lanes.includes('blocked_or_needs_review'),
);
...
const counts = {
  ...
  nc_overlap_rows: rowsWithNcOverlap.length,
  nc_overlap_occurrences: sum(rowsWithNcOverlap, (row) => row.occurrences),
  blocked_overlap_rows: rowsWithBlockedOverlap.length,
  blocked_overlap_occurrences: sum(rowsWithBlockedOverlap, (row) => row.occurrences),
  triple_overlap_rows: rowsWithTripleOverlap.length,
  triple_overlap_occurrences: sum(rowsWithTripleOverlap, (row) => row.occurrences),
  ...
  agent6_source_family_boundary_required_rows: worksetRows.length,
...
const artifact = {
  schema_version: 1,
  artifact_type: 'agent3_old_dictionary_overlap_candidate_use_boundary_workset',
  generated_at: new Date().toISOString(),
  generator: 'scripts/build_agent3_old_dictionary_overlap_candidate_use_boundary_workset.mjs',
  lane_owner: 'Agent 3',
  status: 'evidence-ready',
  target:
    'NC and blocked-review overlap candidate-use boundary workset extracted from old-dictionary boundary triage navigation',
  inputs: {
  ...
  authority_boundary: {
    ...
    overlap_membership_only: true,
    source_family_selection_boundary_required: true,
...
console.log(
  `Agent 3 overlap workset rows=${counts.workset_rows} occurrences=${counts.workset_occurrences} rids=${counts.unique_source_rids}`,
);
...
function buildWorksetRow(row) {
  ...
  return {
    row_id: `agent3-overlap-candidate-use-boundary-${row.queue_id}`,
    ...
    workset_role: 'agent6_source_family_selection_boundary_candidate_exact_navigation_row_not_transform_authority',
...
function buildBlockerRows(rows) {
  ...
  return [...grouped.entries()]
    ...
    .map(([exactBlocker, blockerRows]) => ({
      row_id: `agent3-overlap-candidate-use-blocker-${sha256(exactBlocker).slice(0, 12)}`,
      ...
      queue_id_sample: blockerRows.slice(0, 12).map((row) => row.queue_id),
      token_id_sample: blockerRows.slice(0, 12).map((row) => row.token_id),
      handoff_owner: 'Agent 10 for package intake; Agent 6 for source-family boundary review if routed',
      status: 'exact_overlap_blocker_preserved_no_transform_authority',
...
function buildTriageGroupRows(rows) {
  ...
  return [...grouped.entries()]
    ...
    .map(([triageGroup, triageRows]) => ({
      row_id: `agent3-overlap-candidate-use-triage-${triageGroup}`,
      triage_group: triageGroup,
      workset_rows: triageRows.length,
      workset_occurrences: sum(triageRows, (row) => row.occurrences),
      ...
      source_rid_prefix_rows: new Set(triageRows.flatMap((row) => row.source_rid_prefixes)).size,
      exact_blockers: [...new Set(triageRows.map((row) => row.exact_blocker))].sort(),
      queue_id_sample: triageRows.slice(0, 12).map((row) => row.queue_id),
      token_id_sample: triageRows.slice(0, 12).map((row) => row.token_id),
      status: 'triage_group_navigation_only_agent6_boundary_required',
...
function buildSourceFamilyRows(rows) {
  ...
  return [...grouped.entries()]
    .sort(([left], [right]) => left.localeCompare(right))
    .map(([sourceFamily, entries]) => {
      const rowsForFamily = entries.map((entry) => entry.row);
      return {
        row_id: `agent3-overlap-candidate-use-source-family-${slug(sourceFamily)}`,
        ...
        triage_groups: [...new Set(rowsForFamily.map((row) => row.triage_group))].sort(),
        queue_id_sample: rowsForFamily.slice(0, 12).map((row) => row.queue_id),
        token_id_sample: rowsForFamily.slice(0, 12).map((row) => row.token_id),
...
function buildSourceFamilySetRows(rows) {
  ...
  return [...grouped.entries()]
    ...
    .map(([sourceFamilySetKey, familyRows]) => ({
      row_id: `agent3-overlap-candidate-use-source-family-set-${sha256(sourceFamilySetKey).slice(0, 12)}`,
      source_family_set: sourceFamilySetKey.split(' | ').filter(Boolean),
      workset_rows: familyRows.length,
      workset_occurrences: sum(familyRows, (row) => row.occurrences),
...
function buildBucketSourceFamilySetRows(rows) {
  const grouped = groupBy(rows, (row) => `${row.bucket_id || 'missing'}||${row.source_family_set_key}`);
  return [...grouped.entries()]
    .sort(([left], [right]) => left.localeCompare(right))
    .map(([key, familyRows]) => {
      const [bucketId, sourceFamilySetKey] = key.split('||');
      return {
        row_id: `agent3-overlap-candidate-use-bucket-family-set-${sha256(key).slice(0, 12)}`,
        bucket_id: bucketId,
        source_family_set: sourceFamilySetKey.split(' | ').filter(Boolean),
        workset_rows: familyRows.length,
        workset_occurrences: sum(familyRows, (row) => row.occurrences),
        source_rid_references: sum(familyRows, (row) => row.source_rid_count),
        unique_source_rids: new Set(familyRows.flatMap((row) => row.source_rids)).size,
        exact_blocker: familyRows[0]?.exact_blocker || null,
        triage_groups: [...new Set(familyRows.map((row) => row.triage_group))].sort(),
        queue_id_sample: familyRows.slice(0, 12).map((row) => row.queue_id),
        token_id_sample: familyRows.slice(0, 12).map((row) => row.token_id),
        status: 'bucket_source_family_set_navigation_only_boundary_required_before_transform',
        dedupe_key: sha256([key, familyRows.length, sum(familyRows, (row) => row.occurrences)].join('|')),
      };
    });
}
...
function buildSourceRidPrefixRows(rows) {
  ...
  return [...grouped.entries()]
    ...
    .map(([prefix, entries]) => {
      ...
      return {
        row_id: `agent3-overlap-candidate-use-source-rid-prefix-${slug(prefix)}`,
        ...
        triage_groups: [...new Set(rowsForPrefix.map((row) => row.triage_group))].sort(),
        queue_id_sample: rowsForPrefix.slice(0, 12).map((row) => row.queue_id),
        token_id_sample: rowsForPrefix.slice(0, 12).map((row) => row.token_id),
...
function writeMarkdown(filePath, artifact) {
  const lines = [
    ...
    `- Source-family rows / source-family sets / bucket-family sets: ${artifact.counts.source_family_rows}/${artifact.counts.source_family_set_rows}/${artifact.counts.bucket_source_family_set_rows}`,
    ...
    '|---|---:|---:|---:|---:|---:|',
    ...artifact.triage_group_rows.map(
      (row) =>
        `| ${row.triage_group} | ${row.workset_rows} | ${row.workset_occurrences} | ${row.source_family_set_count} | ${row.source_rid_references} | ${row.unique_source_rids} |`,
    ),
    '',
    '## Blockers',
    '',
    ...
    ...artifact.blocker_rows.map(
      (row) =>
        `| ${row.exact_blocker} | ${row.workset_rows} | ${row.workset_occurrences} | ${row.source_family_set_count} | ${row.source_rid_references} | ${row.status} |`,
    ),
    '',
    '## Stop Condition',
    '',
...
function triageRank(triageGroup) {
  return {
    commercial_clean_nc_overlap: 1,
    commercial_clean_blocked_overlap: 2,
    commercial_clean_nc_blocked_overlap: 3,
  }[triageGroup] || 99;
}
