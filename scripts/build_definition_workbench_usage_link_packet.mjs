...
function buildCounts(sampleRows, usageRows) {
  ...
  return {
    ...
    sample_forbidden_verified_label_rows: sampleRows.filter((row) => row.review_status === 'verified' || row.status === 'verified').length,
...
function writeReport(relativePath, artifact) {
  const lines = [
    ...
    '',
    `Definition sample boundary status: ${artifact.definition_sample_contract.publication_boundary?.boundary_status || '(missing)'}`,
    `Definition sample clears publication readiness: ${artifact.definition_sample_contract.publication_boundary?.clears_publication_readiness}`,
    `Definition sample reviewed lexical authority: ${artifact.definition_sample_contract.publication_boundary?.reviewed_lexical_authority}`,
    `Definition sample publication claim: ${artifact.definition_sample_contract.publication_boundary?.publication_claim}`,
    '',
