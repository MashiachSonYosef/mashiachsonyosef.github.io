...
const outputJsonPath = 'reports/agent1-workbench-cc-by-attribution-boundary-map-2026-06-04.json';
const outputMdPath = 'reports/agent1-workbench-cc-by-attribution-boundary-map-2026-06-04.md';
const contractJsonPath = 'reports/agent1-spark1-pipeline-contract-workbench-cc-by-attribution-boundary-map-2026-06-04.json';
const contractMdPath = 'reports/agent1-spark1-pipeline-contract-workbench-cc-by-attribution-boundary-map-2026-06-04.md';
...
const declared = sourcePartitions.license_partition_counts?.['CC-BY'];
const ccBy = (sourcePartitions.top_partitions || []).filter((partition) => partition.license_label === 'CC-BY');
const sampledRows = ccBy.reduce((sum, partition) => sum + (partition.source_row_count || 0), 0);
const sampledSourceIds = ccBy.reduce((sum, partition) => sum + (partition.unique_source_id_count || 0), 0);
const sampledWorks = ccBy.reduce((sum, partition) => sum + (partition.unique_work_count || 0), 0);

const rows = ccBy.map((partition) => ({
  ...
  blocker_reason: 'CC-BY source-name partition requires attribution boundary treatment before public display, answer use, definition text use, or package/export behavior.',
  sample_source_urls: partition.sample_source_urls || []
}));

const artifact = {
  schema_version: 1,
  artifact_type: 'agent1_workbench_cc_by_attribution_boundary_map',
  generated_at: '2026-06-04T23:45:00.000Z',
  generator: 'scripts/build_agent1_workbench_cc_by_attribution_boundary_map.mjs',
  ...
  counts: {
    declared_cc_by_partition_count: declared?.partition_count ?? null,
    declared_cc_by_source_row_count: declared?.source_row_count ?? null,
    sampled_cc_by_partition_count: rows.length,
    sampled_cc_by_source_row_count: sampledRows,
  ...
  exact_blocker: {
    id: 'cc_by_attribution_boundary_required',
    blocker_reason: 'CC-BY source-name partitions require attribution boundary treatment before source/license custody acceptance, public display, answer use, definition text use, or package/export behavior.',
    missing_fields: [
  ...
  handoff: {
    agent10: 'Consume as Agent 1 CC-BY attribution boundary evidence for release/package intake.',
    spark1: 'May rerun this deterministic subset build and validator only; do not authorize export or display.',
    agent6: 'Route only through Agent 10 with exact CC-BY row/subset boundary question.'
  },
  zero_output_counts: {
    answer_rows: 0,
    source_rows: 0,
  ...
  non_acceptance_boundary: {
    ...
    no_cc_by_export_authorization: true,
...
const contract = {
  ...
  inputs: [inputPath],
  command_or_script: {
    build: 'node scripts/build_agent1_workbench_cc_by_attribution_boundary_map.mjs',
  ...
  validator: {
    command: 'node scripts/validate_agent1_workbench_cc_by_attribution_boundary_map.mjs',
    contract_validator: 'node scripts/validate_agent1_spark1_workbench_cc_by_attribution_boundary_contract.mjs',
  ...
  agent6_boundary_need: 'Agent 6/legal boundary is required before CC-BY source/license custody acceptance, public display, answer use, definition text use, or package/export behavior.',
...
writeFile(outputMdPath, `# Agent 1 Workbench CC-BY Attribution Boundary Map - 2026-06-04
...
- declared CC-BY partitions: \`${declared?.partition_count ?? 'null'}\`
- declared CC-BY source rows: \`${declared?.source_row_count ?? 'null'}\`
- sampled top-partition CC-BY partitions: \`${rows.length}\`
- sampled top-partition CC-BY source rows: \`${sampledRows}\`
...
writeFile(contractMdPath, `# Agent 1 / Spark-1 Pipeline Contract - Workbench CC-BY Attribution Boundary Map - 2026-06-04
