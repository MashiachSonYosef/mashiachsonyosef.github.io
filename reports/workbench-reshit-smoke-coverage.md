# Reshit Smoke Coverage

Generated: 2026-05-31T10:53:28.385Z

## Scope

- Candidate evidence: .local-cache/workbench-evidence/full/reshit-candidate-evidence.json
- Target queue: .local-cache/workbench-evidence/smoke-target-queue.json
- Known nonzero source files: 271
- Queued source files: 271
- Covered source files: 271
- Uncovered source files: 0
- Queued outside known nonzero: 0
- Suggested gap targets: 0

## Known Nonzero Counts

- Supported: 339
- Candidate: 1351
- Weak: 700
- Ambiguous: 2064

## Uncovered Counts

- Supported: 0
- Candidate: 0
- Weak: 0
- Ambiguous: 0

## Suggested Gap Targets

| slug | source files | supported | candidate | weak | ambiguous |
|---|---:|---:|---:|---:|---:|

## Uncovered Source Files

| source file | work | supported | candidate | weak | ambiguous | best raw |
|---|---|---:|---:|---:|---:|---:|

## Boundary

This report reads existing reshit candidate evidence and the smoke target queue only. It does not scan broad corpus files, expand prefix families, rank definitions, or choose HUD winners.
