# Agent 2 Token-Source Nested Gate-Proof Consumption Receipt

Generated: 2026-06-05T15:44:00.000Z

| target | required Agent 1 fields | transform action once classified | exact blocker if not classified | handoff owner | stop condition |
| --- | --- | --- | --- | --- | --- |
| Agent2 token-source nested gate-proof consumption | metadata-only aggregate chain plus current Agent4 validator proof | consume as validator/prereq evidence only; no candidate-use or transform rows | no_candidate_rows_or_candidate_use_packet_from_aggregate | Agent 2 definer retains metadata-only aggregate evidence; Agent10/Agent6 require a separate exact candidate-use packet before downstream use. | Stop at nested gate-proof consumption. Do not derive candidate text, definition content, answer rows, public output, route writes, accepted text, export rows, or release artifacts from aggregate metadata or nested proof evidence. |

## Counts

- Aggregate edge rows: 1951013.
- Matched token occurrences: 49791095.
- Consumed gate-proof commands passed: 1.
- Candidate/definition/lemma/reader-hint/answer/public/route/runtime rows: 0.

## Excluded Stale Proof

- `reports/agent4-agent2-state-integrity-rollup-gate-proof-2026-06-05.json`: validated pre-correction state integrity rollup counts and is not consumed by this receipt.

## Blockers Preserved

- `no_candidate_rows_or_candidate_use_packet_from_aggregate`
- `aggregate_is_nonpublic_token_source_partition_metadata_only`
- `separate_exact_boundary_required_for_any_candidate_use_answer_public_runtime_or_release_use`

## Non-Acceptance Boundary

- No QA acceptance
- No Definition authority
- No answer acceptance
- No source/license/legal acceptance
- No accepted gloss/text
- No public/runtime mutation
- No route-shard edit
- No candidate-use authorization
- No candidate text export
- No NC commercial authorization
- No release action

