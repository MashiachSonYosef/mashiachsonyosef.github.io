...
Use a license-first lexical strategy later:

- First preference: Wikidata Lexemes wherever coverage exists, because structured Lexeme data is CC0.
- Biblical Hebrew fallback/enrichment: OpenScriptures morphHB plus OpenScriptures Hebrew Lexicon, preserving CC BY 4.0 attribution.
- Broad reference layer only: Kaikki/Wiktionary, clearly separated from project overlays with CC BY-SA/GFDL attribution.

Do not use unclear or non-commercial datasets for automatic HUD data.

No lexical data source is part of the owner's CC0 English overlay unless that source is itself CC0 or public domain.

Machine draft translation must never use CC BY-SA text as its primary wording source.

## Candidate datasets

| Dataset | URL | License | Format | Fields available | Coverage | Import difficulty | Recommendation |
| --- | --- | --- | --- | --- | --- | --- | --- |
| OpenScriptures Hebrew Bible / morphHB | https://github.com/openscriptures/morphhb | CC BY 4.0 for lemma/morphology; WLC text public domain | OSIS XML; scripts for JSON conversion | Hebrew tokens, immutable word IDs, lemma attributes, morphology codes | Biblical Hebrew | Medium | Use second, after Wikidata coverage is checked, for Tanakh word identity, morphology, and lemma mapping. Not enough by itself for English HUD glosses. |
| OpenScriptures Hebrew Lexicon | https://github.com/openscriptures/HebrewLexicon | CC BY 4.0; underlying BDB/Strong material noted as public domain | XML files: BrownDriverBriggs, HebrewStrong, LexicalIndex, AugIndex | Lexical entries, Strong/BDB mapping, gloss/definition material, augmented Strong mapping | Biblical Hebrew | Medium | Use second, paired with morphHB, for Biblical Hebrew strict renderings and root/lemma support. Preserve CC BY 4.0 attribution. |
...
| Kaikki / Wiktextract Hebrew dictionary | https://kaikki.org/dictionary/Hebrew/index.html | Derived from Wiktionary; Wiktionary text is CC BY-SA 4.0 and GFDL | JSONL download | Headwords, forms, POS, senses/glosses, tags, categories; roots where Wiktionary has them | Mixed Biblical / Rabbinic / Modern, uneven | Medium | Reference layer only. Keep attribution/share-alike obligations isolated from CC0 overlays and do not use as machine-draft primary wording. |
| Raw Wiktionary dumps | https://dumps.wikimedia.org/hewiktionary/latest/ and https://dumps.wikimedia.org/enwiktionary/latest/ | CC BY-SA 4.0 and GFDL for entry text | XML dumps | Full wiki pages/templates/modules; needs extraction | Mixed; depends on edition | High | Reference layer only. Prefer Kaikki first unless we need direct dump reproducibility; do not mix into CC0 overlay output. |
| Wikidata Lexeme dumps | https://dumps.wikimedia.org/wikidatawiki/entities/ | CC0 for structured Lexeme namespace data | JSON, RDF, N-Triples, Turtle | Lemmas, forms, senses, glosses, grammatical statements, external IDs | All languages; Hebrew coverage uneven | High | First preference wherever coverage exists. Best fit for reusable lexical data that can coexist with CC0 overlay work. |
...
1. Check Wikidata Lexeme coverage for the target words first.
2. Add OpenScriptures morphHB plus HebrewLexicon only for gaps or Biblical-Hebrew enrichment.
3. Keep Wikidata-derived CC0 rows and OpenScriptures CC BY 4.0 rows separately attributed in the lexical table.
4. Keep Kaikki/Wiktionary out of the first proof of concept unless it is explicitly approved as a reference-only layer.
5. Add word-level HUD only after the lexical table and attribution model are validated.

This avoids contaminating the owner's CC0 overlay layer with CC BY-SA or unclear dictionary data.

## Proof-of-concept choice

Recommended: C. Wikidata + OpenScriptures with separate attribution.

Reason: Wikidata-only is legally clean but likely too sparse for useful Biblical Hebrew coverage. OpenScriptures-only is strong for Tanakh but starts from CC BY 4.0 instead of CC0. The combined model tests the real architecture: prefer CC0 rows when present, then fall back to attributed OpenScriptures rows when needed, while keeping both outside the CC0 English overlay.
