...
(async () => {
  ...
  const ruledLine = (k, table) => {
    ...
    const parts = surfaces.map((s) => {
      ...
      if (g && table === zone.gloss && carriersOff(s)) return "—";
      return g ? spanJoin(g) : "—";
    });
  ...
  const openHud = async (el, region, unitId, wordPos, opts = {}) => {
    ...
    const textOfCell = (surface) => {
      ...
      if (g && bin === zone && carriersOff(surface)) return "—";
      return g ? spanJoin(g) : "—";
    };
    ...
    const renderCuts = () => {
      ...
      if (foldsUnreached.length) {
        ...
        again.addEventListener("click", async () => {
          if (again.disabled) return;      // one asking at a time: a double tap spliced the fold in twice
          again.disabled = true;
          const gen = myGen;
  ...
  const sourcesOffSay = () => {
    ...
    const keys = new Set([...sourcesOff].map((id) => (rec[id] && rec[id].key) || id));
