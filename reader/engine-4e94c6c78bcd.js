...
(async () => {
  ...
  let POSTURES = null;
  // THE SEVEN POSITIONS, in the reader's own card. The same list the door
  // carries, kept under the same remembered key, so a reader who chose an
  // order on the way in still has it inside the book. Three answer from what
  // is on this disk; four are listed dead with what they wait on, because a
  ...
  //
  // The one toggle that is a statement of this project's rather than a
  // convenience, so it is written out here in full.
  //
  // The Masoretes pointed the text in Tiberias a thousand years after it was
  // written. The pointing is a reading of the letters — one reading, the one
  // that survived with authority — and the letters carry every meaning they
  // are attested to carry, wherever they occur. This site keys every
  // definition on the letters (FRAME rule 7), so the pointing has never been
  // the gate. What this toggle does is let the reader choose whether it is.
  //
  //   keep     the pointing as the page prints it. The card shows the word as
  //            pointed; every reading of the letters is offered; the "reads
  //            first" control orders them — including "masoretic first", the
  //            corpus lane's vowel grade as a sort. Today's behaviour, exactly.
  //   only     the pointing gates. Rows whose source points the word
  //            OTHERWISE (the lattice's VOWEL_MISMATCH) are withheld before the
  //            pool is built; a source whose headword carries no vowels is
  //            silent, not contradicting, and stays; an ungraded row stays,
  //            because unknown is not a mismatch. The card says how many it
  //            withheld. Measured over the 39 books: moves the printed line at
  //            13.2% of graded words, bares 2.0%.
  //   letters  the pointing is lifted, inside the card only. The head shows
  //            the bare consonants with the pointed form faint beside them;
  //            every reading of the letters is offered oldest first, the
  //            grade ignored; a lattice order chosen in "reads first" falls
  //            back to oldest and the rail says so.
  //
  // WHAT NEVER MOVES: the Hebrew on the page. Not a vowel, not a mark, not a
  // nun, not a ketiv or a qere. The Masoretic text is the licensed text and
  // the site changes nothing in it. This toggle reaches the card and the
  // English line under the word, and nothing else — the owner's rule, in his
  // words: "no masoretic markings can be removed from the book page".
  //
  // The DEFAULT is the owner's ruling and is not made here: it ships at
  // "keep" because that is the one position that changes no English on the
  ...
  window.__masorah = masorah;
  const DEF_POS = [
    { id: "corpus", lab: "corpus", live: true,
      say: "Lexicons of the text\u2019s own era answer first \u2014 the corpus lane\u2019s classification of each source, not ours." },
    // The three lattice orders read the corpus lane's grade of every card
    // against the word as this text points it (lattice v12, projected over
    // this book by tools/project-lattice-v12-v1.mjs into <book>.lattice.bin).
    // They are live where that sidecar rides beside the book, fetched the
    // first time one is chosen; a graded card sorts by its tier, an ungraded
    // one after every graded one, and nothing is dropped.
    { id: "masoretic", lab: "masoretic", live: false, needs: "lattice", lattice: "m",
      say: "Readings whose source points the word the way this text points it answer first \u2014 the lattice\u2019s vowel match, the corpus lane\u2019s grade and not ours. The rest keep their own order under them." },
    { id: "cites", lab: "cites here", live: false, needs: "lattice", lattice: "c",
      say: "Readings whose source names this verse answer first \u2014 the lattice\u2019s own citation list for this position. The rest keep their own order under them." },
    ...
    { id: "vowels-differ", lab: "vowels differ", live: false, needs: "lattice", lattice: "x",
      say: "Readings whose source points the word differently while the letters agree answer first, to watch the sense move \u2014 the lattice\u2019s vowel mismatch. The rest keep their own order under them." },
    { id: "outside-era", lab: "outside the era", live: false, needs: "corpus",
      say: "Lexicons the corpus lane classifies as explicitly NOT of the text\u2019s era answer first, to watch the sense move. Unknown is not later: a source with no classification keeps its place." },
    // THE WITNESSED ORDER (the owner selected it 2026-09-25): three columns
    // the corpus lane built over this book, baked as shipped by
    // tools/bake-witnessed-order-v1.mjs — the reading an old witness gives
    // for this word leads, each with its own credit. Live where the book
    // carries the column; the rest of the pool keeps its order under it.
    { id: "witnessed_same_place", lab: "witnessed here", live: false, needs: "witnessed",
      say: "The reading an old source gives at this very verse answers first \u2014 the corpus lane\u2019s witnessed order, same-place column. The rest keep their own order under it." },
    { id: "witnessed_entry", lab: "witnessed in entry", live: false, needs: "witnessed",
  ...
  // THE LATTICE, on demand. <book>.lattice.bin carries, per key, the corpus
  // lane's card fingerprints in the lattice's order with its transliteration
  // flags, and per pointed surface the tier of each card and which cite this
  // verse — lattice-projection-rule-v1. It is fetched the first time an
  // order that needs it is chosen, never before, and a book without one
  // simply keeps those orders dead with their reason.
  let latticeStore = null, latticePromise = null;
  const latticeReady = () => {
    if (!latticePromise) latticePromise = fetchBin(`${BOOK}.lattice`)
      .then((s) => { latticeStore = s; window.__latticeStore = s; return s; })
      .catch((e) => { if (String((e && e.message) || "").startsWith("REFUSED")) { window.__latticeRefused = e.message; console.error(e.message); } return null; });
    return latticePromise;
  };
  // the fingerprint a store row and a lattice card share: FNV-1a over the
  // UTF-16 code units of "text|source label", eight hex digits — the same
  // function, to the character, as tools/project-lattice-v12-v1.mjs
  const fnv1a = (s) => {
    let h = 0x811c9dc5;
  ...
  const licClass = (key) => {
    ...
    if (/^cc_by_sa/u.test(p) || /gfdl/u.test(p)) return 2;   // a GFDL dual reads as share-alike, the corpus lane's class list agrees
  ...
  const TOGGLES = [
    ...
    { id: "masorah", voice: "claim", lab: "the pointing", why: "the Masoretic vowels: kept as the page prints them, made the gate on what a word can mean, or lifted inside the card — the page itself never changes",
      ...
      get: () => masorah,
      set: (id) => {
        if (!MASORAH_POS.includes(id)) return;
        if (id === "only" && !(zone.emitted_from && zone.emitted_from.toggles && zone.emitted_from.toggles.lattice)) return;
        masorah = id; window.__masorah = id;
        try { localStorage.setItem(MASORAH_KEY, id); } catch { /* the choice still stands on this page */ }
        // lifting the Masorah leaves no grade to sort by: a lattice order in
        // "reads first" falls back to oldest, said on the rail by its own word
        if (id === "letters") { const pos = DEF_POS.find((p) => p.id === defOrder); if (pos && pos.needs === "lattice") { defOrder = "oldest"; try { localStorage.setItem(DEF_KEY, defOrder); } catch { /* same */ } } }
        // the card re-opens so its pool is rebuilt under the new position —
        // after the sidecar is here, never before, or it rebuilds unfiltered
        const reopen = () => { repaintGlossOrder(); if (activeEl) { const wb = activeEl; closeHud(); (wb.querySelector(".w span") || wb.querySelector(".w") || wb).click(); } };
    ...
    { id: "licence", voice: "arrange", lab: "license", why: "prefer readings released under one license — a sort, never a filter; a source the site refuses is not in the catalog to be sorted",
      ...
      get: () => licencePref,
      set: (id) => { licencePref = id; try { localStorage.setItem(LIC_KEY, id); } catch { /* the choice still stands on this page */ } if (redrawReadings) redrawReadings(); },
      now: () => ({ any: "any license", pd: "public domain first", by: "CC BY first", "by-sa": "CC BY-SA first" })[licencePref] },
  ...
  try {
    [zone, index, POSTURES, CORPUS_REC, LANG_REC, SHORT_REC] = await Promise.all([
      ...
      // The corpus lane's classification of every witness — which body of
      // Hebrew a source describes. The definition-order toggle's "corpus"
    ...
    // with itself.
    window.__zone = zone;
    // which orders this book can answer: the lattice ones where its sidecar
    // rides beside it, the corpus one where the corpus record loaded — and a
    // remembered choice this book cannot answer falls back to oldest first
    {
      const hasLattice = !!(zone.emitted_from && zone.emitted_from.toggles && zone.emitted_from.toggles.lattice);
      for (const p of DEF_POS) {
      ...
      if (pos && pos.needs === "lattice") latticeReady();
      if (editionMark === "diff") document.body.classList.add("ed-mark");
    }
  } catch (e) {
  ...
  // A DEMONSTRATION IS NOT A BOOK, AND ITS MASTHEAD SAYS SO. The catalog has
  // no entry for a passage and never will; letting the masthead ask for one
  // prints "no name is on record" over something that was never named. The
  // zone's own title is the demonstration's name, and the label above it says
  // what kind of thing this is.
  {
    const ti = (zone.emitted_from || {}).test_instrument;
    const dm = ((zone.emitted_from || {}).walk || {}).demonstration_oracle;
    if (ti && dm) {
      const heRow = heT.closest(".t-row"), enLab = document.getElementById("enLab");
      if (heRow) heRow.style.display = "none";
      if (enLab) enLab.textContent = "a demonstration of";
      enT.textContent = zone.work;
      enT.classList.remove("unnamed");
      enT.removeAttribute("title");
  ...
  const receiptsText =
    `${pageMeasure.c0_off
      ? `${renderedWordTotal.toLocaleString()} positions, ${pageMeasure.words.toLocaleString()} words on the read axis and ${pageMeasure.c0_off.toLocaleString()} scribal marks among them`
  ...
  // the demonstration banner, read from the zone's own receipts; the rule's
  // plain English rides in as a meta from the door, so the record says it
  // once and the page never re-words it
  {
    const ti = (zone.emitted_from || {}).test_instrument;
    const dm = ((zone.emitted_from || {}).walk || {}).demonstration_oracle;
    if (ti && dm) {
      const el = document.getElementById("demoLine");
      const plain = META("reader-demonstration");
      const lead = document.createElement("b");
      const someTyped = Array.isArray(dm.typed_rows) && dm.typed_rows.length;
      lead.textContent = dm.carried
        ? (someTyped ? "A demonstration, carried, with the mark typed" : "A demonstration, carried")
        : "A demonstration, typed";
      el.append(lead, " \u2014 ");
      el.append(plain ? `${plain} ` : "");
      el.append(dm.carried
        ? `The ink of this passage is ${dm.carried.work_id} ${dm.carried.unit_id} as its source wrote it${dm.carried.credit && dm.carried.credit.line ? "" : ""}. `
        : `No source this project holds carries this mark, so the ink here was typed: ${(dm.typed || {}).why || ""}. `);
      if (someTyped) el.append(`${dm.typed_rows.length === 1 ? "One word of it was" : `${dm.typed_rows.length} words of it were`} typed and marked as typed: ${dm.typed_rows_why || "no source this project holds writes the mark here"}. `);
      if (!dm.carried && (dm.typed || {}).where_the_wire_goes) el.append(`When the record lands it arrives at ${dm.typed.where_the_wire_goes}, and this passage is replaced by the book. `);
      el.append("Nothing on this page is served as a work.");
      if (dm.carried && dm.carried.credit && dm.carried.credit.line) {
        const cr = document.createElement("span"); cr.className = "demo-credit";
        cr.textContent = ` ${dm.carried.credit.line}`;
        el.append(cr);
      }
      el.hidden = false;
    }
  ...
  const routesFor = async (key) => {
    ...
    if (!shardCache.has(shard)) {
      ...
      // A SHARD THAT DID NOT ARRIVE IS NOT AN EMPTY SHARD. A failed fetch was
      // cached as null for the life of the page, and every key in that shard
      // then read "no exact route in the catalog for this form" — an honest
      // absence claimed for a network fault, on a phone between two cells or
      // under a deploy in progress, and never asked again (the owner's
      // screenshot, 2026-09-25: the card for a word the line reads). The
      // failure leaves the cache and is thrown, so the caller says what
      // happened and can ask again.
      shardCache.set(shard, fetch(url)
        .then((r) => { if (!r.ok) throw new Error(`${r.status}`); return unpack(r); })
        .catch((e) => { shardCache.delete(shard); throw new Error(`SHARD_UNREACHABLE ${shard}: ${(e && e.message) || e}`); }));
    }
  ...
  // whether every carrier of a key's baked reading is switched off
  const carriersOff = (k) => { const gm = sourcesOff.size && zone.gloss_m ? zone.gloss_m[k] : null; return !!(gm && Array.isArray(gm.by) && gm.by.length && gm.by.every((id) => sourcesOff.has(id))); };
  // what a key's baked column reads under the switches: its reading where a
  // carrier is on; its baked alternate where that one has a carrier on; a
  // dash where the reader turned every carrier of both off
  const switchedReading = (k, g) => {
    if (!g) return "\u2014";
    if (!carriersOff(k)) return spanJoin(g);
    const alt = zone.gloss_m[k].alt;
  ...
  const ruledLine = (k, table) => {
    ...
    const parts = surfaces.map((s) => {
      ...
      // a baked reading whose every carrier the reader switched off is not
      // read here either: a source turned off is not quoted on the line;
      // its baked alternate stands where one of its carriers is on
      if (g && table === zone.gloss) return switchedReading(s, g);
      return g ? spanJoin(g) : "—";
    });
    // a rung of a run's lattice that reads nothing — the run joined, pressed
    // on the card, with no dictionary giving it — is not a ruling for the
    // line: the line stood as "—" under the run the moment the rung was
    // pressed, before the catalog had answered, and stayed so when the card
    // closed. The words' own lines stand instead.
    if (k.includes("\u05be") && parts.every((x) => x === "—")) return null;
    return parts.join(" + ");
  };

  /* ── THE ORDER TOGGLES ────────────────────────────────────────────────────
     Where the ink offers more than one reading at one position, the reader
     sets which one leads and the page says whose order it is. Nothing is
     added and nothing is dropped: an ORDER toggle is never a selection, and
     the Hebrew on the page never moves — C0 never changes.

     TWO SWITCHES ARE SELECTIONS, by the owner's ruling of 2026-09-17, and
     each one says on the card how many records it withheld: the masorah
     ...
     toggle; then the order the record itself arrived in. Where the side the
     ...
     definitions by era, by pointing and by which source names this verse —
     read fields no record on this side carries yet; they arrive with the
     corpus lane's word layer, and this switch is the frame they plug into. */
  const ORDER_KEY = "fh.order.kq";
  const KQ_ORDERS = ["KETIV", "QERE", "SOURCE"];
  const KQ_SAYS = { KETIV: "ketiv first", QERE: "qere first", SOURCE: "as the source sets them" };
  let kqOrder = (() => { try { return KQ_ORDERS.includes(localStorage.getItem(ORDER_KEY)) ? localStorage.getItem(ORDER_KEY) : "KETIV"; } catch { return "KETIV"; } })();
  const KQ_STANDS = [];
  // AND THE SAME LEDGER FOR ORDINARY WORDS. The pairs ledger exists so the
  // order toggle can repaint a pair without rebuilding the page; every other
  // word needs the same thing for the same reason, now that an order changes
  // which reading stands under it and not only which reading leads its card.
  // What is kept is the least a repaint needs: the element and the keys whose
  // readings compose its line. A word whose line came from a ruling is not
  ...
  const kqPick = (regions, parts, table) => {
    ...
    if (usable(wanted)) return { i: wanted, fellFrom: null };
    const alt = regions.findIndex((r, i) => i !== wanted && usable(i));
    if (alt < 0) return { i: -1, fellFrom: null };
    const named = (r) => (r && r.role === "KETIV" ? "the ketiv" : r ? "the qere" : "the half you asked for");
    return { i: alt, fellFrom: named(regions[wanted]) };
  };
  /** The witness chip that rides a reading. Built in one place because the
      order toggle repaints readings, and a reading whose chip stayed behind
      would be wearing another witness's terms. */
  const licChipFor = (ks) => {
    if (!ks || !ks.length || !zone.gloss_m) return null;
  ...
  // readable.
  // Its id keeps the spelling it was declared under. A rule id is a NAME,
  // not prose: respelling it does not correct the record, it replaces the
  // record with a different one that no check, ledger or export refers to.
  // The American-spelling sweep of 2026-09-09 stashed colour-role-rule-v1
  // for exactly this reason and missed this one, and check-licence-leaves
  // caught it by still asking for the name that was declared.
  const LICENCE_RULES = {
  ...
  const placeHud = (el) => {
    ...
    // FIT FIRST, MOVE ONLY IF IT STILL WILL NOT FIT. The card gives up height
    // rather than position — it says so below and it meant it — but it was
    // measuring the room only AFTER deciding to scroll, so a card asking for
    // four fifths of the screen (709px of a 900px phone) made the word's foot
    // have to be inside the top fifth, and nearly every press moved the page.
    // Measured: pressing a word scrolled the document 346px, so every word on
    // screen jumped that far from the finger that pressed it. Nothing was
    // reflowing — the card is fixed and the document did not change height —
    // the page was simply being moved out from under the reader to make room.
    // So the card is capped to the room already beneath the word first. When
    // that room can hold it, the page does not move at all; when it cannot,
    // the scroll below runs exactly as it did, and is now the rare case it
    // was written for. The owner, 2026-09-07: opening a hud moves the Hebrew
    // under it, very confusing.
    const FLOOR = 200;
    const roomNow = window.innerHeight - pad - rect.bottom - pad;
    // set whether or not the card looks like it needs it: at this moment the
    // card is a shell whose readings have not rendered, so asking "is it taller
    // than the room" reads 29px and takes no cap, and the grown card then goes
    // past the room it was never measured against. The block below already
    // learned this on 2026-08-30; this one inherited the lesson by making the
    // same mistake first.
    if (roomNow >= FLOOR) {
      hud.style.maxHeight = `${Math.floor(roomNow)}px`;
      // the same law as below: a card that cannot hold a row of every band at
      // that height does not hold them at that height, and takes the scroll
      // instead of standing there with dead controls in it
  ...
  const poolFor = async (surface) => {
    ...
    // says how many. A page that filtered readings after the pool was built
    // would be a page deciding what a source said; withholding the row is
    // declining to ask that source about this pointing. Rows the lattice did
    ...
    if (pointingNow(surface === openKey ? openPointed : null) === "only") {
      ...
      // built. The first press of "only" found exactly that: 0 withheld of
      // 173 on a word with mismatch cards, because the card re-opened
      // before the lattice landed.
  ...
  // never a filter), then names-as-sound, then the chosen order, then oldest
  // first. A lattice order reads the corpus lane's grade of each record for
  // the word standing open; a record the lattice did not grade sorts after
  // every graded one, and a card whose surface the lattice never graded
  // (a cell of a form, a book without a sidecar) falls through to oldest.
  const sortPool = (list, surface = null) => {
    const pos = DEF_POS.find((p) => p.id === defOrder) || {};
    const gr = latticeStore && latticeStore.grades && openPointed && surface && surface === openKey ? latticeStore.grades[openPointed] : null;
    const lat = gr && latticeStore.routes[gr.k] ? latticeStore.routes[gr.k] : null;
    const fpIdx = lat ? new Map(lat.f.map((fp, i) => [fp, i])) : null;
    const idxOf = (row) => {
      if (!fpIdx) return -1;
    ...
    const primary = (a, b) => {
      if (defOrder === "characters") return a.ledger - b.ledger;
      if (defOrder === "corpus") return era(a) - era(b);
      if (defOrder === "outside-era") return outside(a) - outside(b);
      // a witnessed order leads with the reading its column gives this key —
      // the line's own reading, so the card's first pill is the line
      if (pos.needs === "witnessed") return witnessed(a) - witnessed(b);
  ...
  // already be selected for you." Both halves print, both halves stand in the
  // head of the one card as they always did; the pairs switch only says which
  // one the card opens on and which one backs the English. A half the reader
  // presses in the head is remembered for that place until the switch moves.
  ...
  const openHud = async (el, region, unitId, wordPos, opts = {}) => {
    ...
    // the lattice's grades are keyed by the pointed surface of the form that
    // stands open, and apply to its whole form only (bin === zone: a
    // commentary's or a dictionary's words are not positions of this book)
    openPointed = bin === zone && region && region.s ? region.s : null;
    openKey = bin === zone && region ? (region.form_k || region.k || null) : null;
    ...
    if (regions.length > 1) {
      ...
      regions.forEach((r, i) => {
        ...
        // on a pair, the half that is not open is one press away, here in the
        // head where it already stands; the card then reads from it
        if (!lit && opts.kqOpenAt) {
          seg.setAttribute("role", "button"); seg.tabIndex = 0; seg.style.cursor = "pointer";
          seg.title = r.role === "KETIV" ? "open the ketiv \u2014 the written form" : "open the qere \u2014 the read form";
          seg.addEventListener("click", (ev) => { ev.stopPropagation(); opts.kqOpenAt(i); });
        }
        b.append(seg);
      });
      ...
      // The page above is untouched: this is the head of a card, not the text.
      const bare = document.createElement("span"); bare.textContent = region.form_k || region.k;
      const pointed = document.createElement("span"); pointed.textContent = word.s;
      pointed.style.color = "var(--faint)"; pointed.style.marginInlineStart = "0.5em"; pointed.style.fontSize = "0.8em";
      pointed.title = "as the page points it";
      b.append(bare, pointed);
      b.dataset.masorah = "letters";
    ...
    // THE MASORAH, said on the card in words. A card whose readings were
    ...
    // this file's coinage; what follows the dash is the plain English pair.
    // The owner's ruling, 2026-08-31, in his words: "written read is good
    // plain english with the slash seperator" — the meaning IS the
    // opposition, so the two labels are one juxtaposition and not two
    // sentences. They read nearly identically before, and the qere's said
    // both "reads" and "as written" in the same breath.
    if (region && region.role === "KETIV") {
      const roleLine = document.createElement("p"); roleLine.className = "kq-role";
      roleLine.textContent = "ketiv \u2014 written";
      head.append(roleLine);
    } else if (region && region.role === "QERE") {
      const roleLine = document.createElement("p"); roleLine.className = "kq-role";
      roleLine.textContent = "qere \u2014 read";
      head.append(roleLine);
    }
    ...
    // RULE 2 (owner, 2026-09-02): a maqaf compound is one C0 per word, each
    // its own card, and the joiner rides as ink on the word before. The card
    // says so in plain English, in one line, on both words of the pair; it
    // groups nothing over them, because the count says two.
    if (word.overlay) {
      const oLine = document.createElement("p"); oLine.className = "kq-role";
      oLine.textContent = `the opening words this comment is about \u2014 word ${word.overlay.at + 1} of ${word.overlay.of}`;
      head.append(oLine);
      const oKey = document.createElement("p"); oKey.className = "kq-key";
      oKey.textContent = word.overlay.why || "The printers set this run apart. It is recorded as a run over the words, not as anything inside them: the ink is exactly what the source wrote, and the record says which words it covers.";
      head.append(oKey);
    }
    if (word.typed_here) {
      const tLine = document.createElement("p"); tLine.className = "kq-role";
      tLine.textContent = "typed here, not carried";
      head.append(tLine);
      const tKey = document.createElement("p"); tKey.className = "kq-key";
      tKey.textContent = "Every other word of this passage is the source's own ink. This one is not: no source this project holds writes this mark here, so it was typed to show the rule, and it is marked as typed wherever it appears.";
      head.append(tKey);
    }
    if (word.mark) {
    ...
    if (Array.isArray(word.letter_marks) && word.letter_marks.length) {
      ...
      lKey.textContent = "A mark on the letter, not on the word: the scribes wrote the letter apart from its neighbors, and the page draws it as they drew it. The word keeps its place in the count and its key.";
      head.append(lKey);
    }
    // the source's own flag on a word it prints in one form while marking it
    // a ketiv-qere of spelling; nothing is chosen and no second form is shown
    if (word.kq_trivial) {
      const tLine = document.createElement("p"); tLine.className = "kq-key";
      tLine.textContent = "The source marks this word as a ketiv-qere of spelling and prints it in one form; the page prints that form and chooses nothing.";
      head.append(tLine);
    }
    ...
    if (mqNext || mqPrev) {
      const mqLine = document.createElement("p"); mqLine.className = "kq-role";
      mqLine.textContent = (mqNext && mqPrev
        ? "joined by a maqaf to the words before and after"
        : mqNext ? "joined by a maqaf to the next word" : "joined by a maqaf to the word before")
        + " \u2014 one breath, each word its own card";
      head.append(mqLine);
    }
    ...
    // a mark asks the catalog nothing: the card is its head, and the head has
    ...
    let foldsAsking = false;   // one asking at a time, whatever button the row was redrawn with
    ...
    if (opts.run) {
      ...
      const forms = runWholeForms(keys);   // as written, joined, then the folds
      // THE LATTICE HOLDS ITS THREE RUNGS WHATEVER THE SHELF ANSWERS (the
      // owner, 2026-09-25: "missing AMB and AB in megacompspan"): the run as
      // written, the run joined into one, and the words one by one are always
      // on the card. A rung no dictionary here answers says so when pressed;
    ...
    if (opts.run && !mem.cut) {
      // the as-written rung is always on the card now; the card opens on it
      // only where the shelf reads it (the line's own test: the baked table)
      const asWritten = bin.gloss && bin.gloss[region.k] ? covers.find((c) => c.length === 1 && c[0].surface === region.k) : null;
    ...
    const textOfCell = (surface) => {
      ...
      if (g && bin === zone) return switchedReading(surface, g);
      return g ? spanJoin(g) : "—";
    };
    ...
    const holdAndPaint = (box, line) => {
      ...
      // The license chip stays with a reading that has not changed: an open
      // that paints the same words back was dropping the chip with them, and
      // the reading and its license are never to separate (owner, 2026-08-30).
      ...
      if (box.style.height && box.scrollHeight > box.clientHeight + 1) {
        ...
        // the chip's line rides along whole when one is present — grown
        // height owes the text its lines AND the chip its own row
        const chipEl2 = box.querySelector(".g-lic");
        const chipH2 = chipEl2 ? chipEl2.getBoundingClientRect().height + (parseFloat(getComputedStyle(chipEl2).marginTop) || 0) : 0;
        // GROW UNTIL IT IS WHOLE, don't predict it whole. One pass computed
        // the lines from the height the text wanted BEFORE the box changed,
        // and a box that grows re-wraps: at the second word of Genesis the
        // arithmetic landed on 146px for content that then reflowed to 147,
        // and the reader's own chosen reading was cut by three pixels — the
        // one thing this box
        // exists to prevent. So it is measured again after each growth and
        // grows another line while it is still short. The bound is the
        // reading's own lines and nothing is ever shrunk.
      ...
      // A word inside a maqaf run does not print its own reading — the run
      // prints them once, together. So a ruling repaints the word's hidden
      // element as it always did, and the run's line is made again from it.
      const run = box.closest && box.closest(".wjoin");
    ...
    const paintGloss = () => {
      ...
      if (!ruled) {
        ...
        // on a pair, the provenance mark follows the half that is open: the
        // English now reads from it, and both ends of the claim wear the mark
        if (opts.kqEls && regionIndex >= 0) {
          opts.kqEls.forEach((el, j) => el.classList.toggle("backs-en", j === regionIndex));
          glossEl.classList.add("backs-en");
        }
        return;
      }
    ...
    const renderCuts = () => {
      ...
      covers.forEach((c) => {
        ...
        btn.addEventListener("click", () => {
          ...
          if (!(opts.run && c.length === 1)) { mem.cut = cutKey(c); markRuled(); } else pressedCut = c;
      ...
      if (foldsUnreached.length) {
        ...
        again.disabled = foldsAsking;
        again.addEventListener("click", async () => {
          if (foldsAsking) return;         // one asking at a time: a double tap spliced the fold in twice
          foldsAsking = true; again.disabled = true;
          const gen = myGen;
    ...
    const renderReadings = async () => {
      ...
      if (!pool || !pool.length) {
        ...
        if (pool && (pool.withheld || pool.withheldBySources)) {
          const bits = [];
          if (pool.withheldBySources) bits.push(`${pool.withheldBySources} record${pool.withheldBySources === 1 ? "" : "s"} withheld by your source switches — ${sourcesOffSay()}`);
          if (pool.withheld) bits.push(`${pool.withheld} record${pool.withheld === 1 ? "" : "s"} withheld by the pointing — their source points this word otherwise`);
          p.className = "kq-role sources-withheld";
          p.textContent = `${bits.join("; ")}. No record is left to read here. Nothing here says the shelf lacks it.`;
          readRow.append(p);
          dSlot.replaceChildren(); selectedRecordRef.m = null;
        ...
        p.textContent = runWhole
          ? (surface === region.k
            ? "No dictionary on this shelf gives the run as written, maqaf and all. The words one by one are a press away."
            : "No dictionary on this shelf gives the run joined as one word. The words one by one are a press away.")
          : pool ? "No displayable route in the catalog for this form." : "No exact route in the catalog for this form.";
        readRow.append(p);
        // THE RECORD SLOT IS THIS BLOCK'S. A block with no reading stands on
        // no record, and the last block's record was left standing under it —
        // "he will rule as God", with its cut note, under a block that said it
        // had no route (the owner's screenshot, 2026-09-25).
        dSlot.replaceChildren(); selectedRecordRef.m = null;
        // a division nothing can be read under is not a ruling to remember,
        // and the run's line goes back to its words wherever the run stands
        if (runWhole && mem.cut === cutKey(cover)) {
          mem.cut = null;
          for (const r of document.querySelectorAll(".wjoin")) if (r.__key === region.k) { if (!mem.cells.size) r.classList.remove("chosen"); refreshJoinGloss(r); }
        }
        paintNow(cover.map((c) => textOfCell(c.surface)).join(" + "));
        clampHud(); return;
      }
      // How many readings there are, on the label, because the row shows about
      // three of them and scrolls the rest. Without the count a reader cannot
      // tell a form carrying three routes from one carrying nineteen, and two
      // looks at the same card from different scroll positions read as two
      ...
      // a run's whole rung the reader PRESSED, now read: it is the reader's
      // division, remembered and painted at every place the run stands. The
      // rung the card merely opened on is not a ruling: opening a word and
      // closing it again changes nothing and leaves no mark.
      if (opts.run && cover.length === 1 && pressedCut === cover && mem.cut !== cutKey(cover)) { mem.cut = cutKey(cover); markRuled(); paintGloss(); }
      if (pressedCut === cover) pressedCut = null;
      ...
      // (hg on the word) — the same reading the line prints
      const lemmaFirst = lookup === "headword" && word && word.hg && region.form_k && surface === region.k
        ? pool.find((r) => String(r.text).toLowerCase() === String(word.hg).toLowerCase()) : null;
      let selected = (remembered && pool.find((r) => routeKey(r) === remembered.key)) || lemmaFirst || pool[0];
      const pills = document.createElement("div"); pills.className = "r-pills";
      // what the sort put first, said on the row so a check can read the
      // order's effect off the page: the first reading's licence class
      ...
      // Drawn once per position: pressing one re-sorts the same pool and
      // redraws it. Nothing is added or removed, so the reading standing under
      // the word is still in the list — it moved, it did not change.
      const drawPills = () => {
        ...
        if (pool.withheldBySources) {
          ...
          sNote.textContent = `${pool.withheldBySources} of ${pool.rows} records withheld by your source switches — ${sourcesOffSay()}`;
      ...
      // not showing. Bring it into the row — the row only, by setting scrollTop
      // on the box itself: scrollIntoView is free to scroll the card and the
      // page with it, and moving the page because a pill was low is a cure
      // worse than the fault. After layout, because whether it is out of sight
      // is a fact about the fitted card.
      requestAnimationFrame(() => {
    ...
    if (runSpan) {
      prov.textContent = `One card for the run: ${comps.length} words, joined by a maqaf · as written, joined into one, and word by word are its divisions; a joined spelling is offered, never chosen: it can be another word`;
    ...
    if (myGen !== hudGen) return;   // a dead card places nothing
    placeHud();
  };
  ...
  // THE RUN'S ONE READING LINE. Inside a joined run every word keeps its own
  // reading element — the export reads it there, and a ruling repaints it
  // there — but it is not drawn there, because a reading is wider than a word
  ...
  const refreshJoinGloss = (run) => {
    ...
    // a word of the run the shelf could not answer marks the run's own line,
    // which is the line the reader sees; the words' lines are hidden
    const unreached = wbs.some((wb) => { const g = wb.querySelector(":scope > .g"); return g && g.dataset.off === "unreachable"; });
    if (unreached) line.dataset.off = "unreachable"; else delete line.dataset.off;
    if (!parts.some((p) => p.t)) {
      line.textContent = " ";
      line.classList.add("bare");
      return;
    }
    line.title = unreached ? "the catalog could not be reached for a word of this run; what your switches leave of it is not known yet" : text;
  ...
  const appendWord = (host, built, word) => {
    ...
    if (host.__wjoin) refreshJoinGloss(host.__wjoin);
    host.__wjoin = null;
    host.append(document.createTextNode(" "));
  };

  // RULE 12, wherever a word's text is appended: the letters are counted
  // alef through tav across the whole surface (as the record counts them),
  // and a marked letter is wrapped with the points that ride on it. `state`
  // carries the running letter index so a site cut into carrier text and
  // regions still counts its letters in one sequence.
  const appendLettered = (el, text, marked, state) => {
    if (!marked || !marked.size) { el.append(document.createTextNode(text)); return; }
    let run = null;
    const flush = () => { if (run) { el.append(run); run = null; } };
    for (const c of [...String(text || "")]) {
      ...
      if (isLetter) {
        flush(); state.li += 1;
        const kind = marked.get(state.li);
        if (kind) { run = document.createElement("span"); run.className = `lm lm-${kind}`; run.title = `letter ${state.li}, written ${kind === "dotted" ? "with a dot above" : kind === "suspended" ? "suspended above the line" : kind} by the scribes`; run.textContent = c; }
        else el.append(document.createTextNode(c));
      } else if (isPoint && run) run.textContent += c;
      else { flush(); el.append(document.createTextNode(c)); }
    }
    flush();
  };
  const markedLettersOf = (word) => {
    const m = new Map();
    for (const lm of (word.letter_marks || [])) for (const at of (lm.letters || [])) m.set(at, String(lm.kind || "").toLowerCase());
    return m;
  };

  // THE LINE UNDER A WORD, WHEN A TOGGLE MOVES IT PAST THE BAKED COLUMN.
  // Two cases, both per position rather than per key, which is why the
  // baked gloss table cannot carry them: under "look up by: the headword"
  // the word's hg — the headword's first reading under lemma-sort v3, where
  // the lattice sorted the stack (project-lattice-v12-v1); and under a
  // lattice order, the first card of that order for this pointed surface,
  // baked into the sidecar. Each comes with its own M, so the chip on the
  // line names the witness that actually carries what the line says. Null
  // where neither applies, and the baked column stands.
  //
  // WHEN BOTH ASK. A reader can hold "look up by: the headword" and a lattice
  // order at once, and the two are asked at different grains: the headword's
  // stack is sorted by lemma-sort v3, the lattice's grades are of the FORM as
  // this text points it. There is no grade of the headword's stack against
  ...
  const lineUnder = (word, table) => {
    ...
    if (masorah === "only" && latticeStore && latticeStore.grades && word.s) {
      const gr = latticeStore.grades[word.s];
      const first = gr && gr.o && gr.o.l;
      if (first) return { text: first[0], m: { lic: licenseName(first[2]), m: first[1], y: first[3] }, why: "masorah" };
      // graded, and nothing survives the filter: the word goes bare here,
      // which the frame calls a finding and not an error
      if (gr && gr.o && Object.prototype.hasOwnProperty.call(gr.o, "l")) return { text: "", m: null, why: "masorah-bare" };
    }
    if (masorah === "letters") return null;
    const pos = DEF_POS.find((p) => p.id === defOrder);
    if (pos && pos.needs === "lattice" && latticeStore && latticeStore.grades && word.s) {
      const gr = latticeStore.grades[word.s];
      const first = gr && gr.o && gr.o[pos.lattice];
  ...
  const wordBlock = (word, table) => {
    ...
    // A MARK IS NOT A WORD OF THE BOOK, and the page says so in how it draws
    // it. The type comes from the zone (tools/zone-lib-v1.mjs markOf), which
    // reads the frame's own list of mark kinds; this file only obeys it. The
    // scribes' section marks may be turned off by a reader, because they are
    // bookkeeping about the page; the inverted nunim may not, because they
    // speak about the text — the owner's rules 4, 5 and 6.
    if (word.typed_here) wb.classList.add("typed-here");
    // where the Leningrad codex spells the word otherwise (the lattice's
    // editions diff): the word carries it, and wears it when the edition
    // toggle asks
    if (word.ld) { wb.dataset.ld = word.ld.kind || "differs"; wb.title = `Leningrad differs${word.ld.kind ? ` (${String(word.ld.kind).replace(/_/gu, " ").toLowerCase()})` : ""}${word.ld.other ? `: ${word.ld.other}` : ""}${word.ld.mam ? ` · MAM: ${word.ld.mam}` : ""}`; }
    // AN OVERLAY OVER THE INK. The eighth rule: a commentary's opening
    // quotation was set apart by its printers, and the record carries that as
    // a run over the words rather than as markup inside them. The page draws
    // the run and says what it is; the ink underneath is untouched.
    if (word.overlay) {
      wb.classList.add("ov");
      wb.dataset.ov = word.overlay.kind;
      if (word.overlay.at === 0) wb.classList.add("ov-first");
      if (word.overlay.at === word.overlay.of - 1) wb.classList.add("ov-last");
    }
    if (word.mark) {
    ...
    if (word.kq && regions && regions.length) {
      ...
      const lmMarked = markedLettersOf(word), lmState = { li: 0 };
      let rest = word.s;
      regions.forEach((r, i) => {
        const at = rest.indexOf(r.s);
        if (at > 0) appendLettered(w, rest.slice(0, at), lmMarked, lmState);
        rest = rest.slice(at + r.s.length);
        const seg = document.createElement("span"); seg.className = "wr";
        appendLettered(seg, r.s, lmMarked, lmState);
      ...
      if (rest) appendLettered(w, rest, lmMarked, lmState);
    } else if (regions) {
      ...
      // RULE 12: the mark is on the letter; the word keeps its count and key
      appendLettered(w, word.s, markedLettersOf(word), { li: 0 });
    ...
    let wasRuled = false;
    let lineMoved = null;      // a toggle moved this line past the baked column, with its own M
    const parts = (regions || (word.k ? [{ k: word.k }] : [])).map((r) => {
      ...
      if (!(regions && regions.length > 1)) { const ov = lineUnder(word, table); if (ov) { lineMoved = ov; return spanJoin(ov.text); } }
    ...
    if (line && !wasRuled && table === zone.gloss && zone.gloss_m) {
      const ks = word.kq && regions ? (kqShown >= 0 ? [regions[kqShown].k] : [])
        ...
        : word.k ? [word.k] : [];
      // a moved line wears the M of what it now says, or no chip at all —
    ...
    // without rebuilding the page. What is kept is what a repaint reads: the
  ...
  const wireWord = (built, word, bin, unitId, wordPos, opts = {}) => {
    ...
    // A MARK OPENS THOUGH IT HAS NO KEY. The frame gives a section mark its
    // own C0 and its own card (rules 4 and 5); what it does not give it is a
    // reading, because a mark is not a word and there is nothing to look up.
    // So it is pressable, its card says what it is, and it stops there.
    if (word.held || (!word.w && !word.k && !word.mark)) { built.wb.style.cursor = "default"; return built.wb; }
    ...
    // A PAIR OPENS ON THE HALF THE SWITCH NAMES. Whichever half is pressed,
    // the card opens on the half the pairs switch names, or the half the
    // reader pressed in the head at this place, or a ruled half, or the half
    ...
    const kqLead = () => {
      ...
      return { i: Math.max(0, wanted) };
    };
    const openAt = (i, target) => {
      ...
      openHud(target, region, unitId, wordPos, {
        ...
        kqOpenAt: built.kqStand ? (j) => { kqPicks.set(site, j); openAt(j, built.regionEls[j] || target); } : null,
  ...
  pick("he");
  // the marks switch: it reveals the scribes' section marks, which the frame
  // holds off by default, and it never touches a mark rule 6 protects
  const marksSwitch = () => {
    const btn = document.getElementById("marksToggle");
    // counted from the zone's own words, every section of it: the page draws
    // sections as the reader reaches them, and a count taken from the DOM
    // named only the marks already drawn (2026-09-06)
    const allMarks = (zone.sections || []).flatMap((s) => (s.words || []).filter((w) => w.mark));
    const n = allMarks.length;
    const off = allMarks.filter((w) => w.mark.toggleable).length;
    ...
    const kinds = [...new Set(allMarks.filter((w) => w.mark.toggleable).map((w) => NAMES[w.mark.kind] || String(w.mark.kind).toLowerCase()))];
    ...
    if (n > off) btn.title = `${n - off} further mark${n - off === 1 ? "" : "s"} on this page cannot be hidden: they speak about the text, not about the page`;
  };
  /* The pair switch. Appears only where the book writes pairs, the same rule
     the marks switch follows, and counts them from the zone rather than from
     the sections drawn so far. Three positions, cycled: what stands today
  ...
  // THE ORDER'S COLUMN, SWAPPED INTO THE TABLE THE PAGE DRAWS FROM.
  //
  // The zone carries the default column whole and the other orders as deltas
  // beside it — only the keys where an order disagrees. So the swap is: put
  ...
  // the positive fact that this order reads that word the same way. An order
  // with no column at all is a position that is not live, and none of those
  // reach here.
  let GLOSS_BASE = null, GLOSS_M_BASE = null;
  const applyGlossOrder = (order) => {
    if (!zone || !zone.gloss) return;
    const cols = zone.gloss_orders || {}, mcols = zone.gloss_m_orders || {};
    if (!GLOSS_BASE) {
      GLOSS_BASE = {}; GLOSS_M_BASE = {};
      const moved = new Set();
      for (const o in cols) for (const k in cols[o]) moved.add(k);
      for (const k of moved) {
        GLOSS_BASE[k] = zone.gloss[k];
        GLOSS_M_BASE[k] = zone.gloss_m ? zone.gloss_m[k] : undefined;
      }
    }
    for (const k in GLOSS_BASE) {
      zone.gloss[k] = GLOSS_BASE[k];
      if (zone.gloss_m) {
        if (GLOSS_M_BASE[k] === undefined) delete zone.gloss_m[k];
        else zone.gloss_m[k] = GLOSS_M_BASE[k];
      }
    }
    ...
    if (col) for (const k in col) {
      ...
      if (zone.gloss_m) {
        ...
        else delete zone.gloss_m[k];
      }
    }
  };

  /** Redraw every enrolled word's line under the table as it now stands. */
  const repaintGlossOrder = () => {
    const runs = new Set();
    for (const st of GLOSS_STANDS) {
      // a one-form word may read past the baked column (the headword's
      // lemma-sorted reading, or a lattice order's first card); the joined
      // parts of a several-form word keep to the column
      // a word the reader ruled on keeps its ruling through a switch: the
      // ruling outranks every toggle (the precedence above), and a repaint
      // from the table painted the table back over it until the card was
      // opened again
      const held = st.ks.length === 1 && st.word && st.word.k ? ruledLine(st.word.k, zone.gloss) : null;
      if (held !== null) {
      ...
      else { st.ge.textContent = " "; st.ge.classList.add("bare"); }
      if (line && zone.gloss_m) {
        const chip = moved ? (moved.m ? chipOfMs([moved.m]) : null) : licChipFor(st.ks.filter((k, i) => parts[i] !== "\u2014"));
  ...
  // the sources off, counted as the rail counts them: one source per chip,
    ...
    const one = async ({ st, moved, need }) => {
      ...
      for (let i = 0; i < st.ks.length; i += 1) {
        ...
        try { pool = await poolFor(k); } catch {
          ...
          // the chip is rebuilt only where the line shows the BAKED reading:
          // a line the switch already moved to its alternate wears that
          // alternate's own chip, which names a carrier still on
          if (!moved) {
          ...
          // a run's visible line is made from its words' lines; it is made again
          const runEl = st.ge.parentElement && st.ge.parentElement.closest && st.ge.parentElement.closest(".wjoin");
          if (runEl) refreshJoinGloss(runEl);
          failed.push(st);
          return;
        }
  ...
  const defSwitch = () => {
    ...
    for (const pos of order) {
      ...
      if (!pos.live) { btn.disabled = true; btn.setAttribute("aria-disabled", "true"); }
        ...
        // word came from the zone's baked column and never moved. It moves now.
  ...
  document.addEventListener("click", (e) => {
    ...
    // the first tap opens a card, and the card both stands over the word and
    // MOVES THE PAGE UNDER IT — thirty pixels on a phone, enough that the
    // second tap lands just off the glyph it was aimed at.
    //
    // This used to fire only when the second tap hit the card, which caught
    // the first of those and not the second. A tap landing on the word block
    // beside the glyph is not on the card, so nothing rescued it and the
    // browser selected what was actually under the finger: the space after
    // the word. A reader double-tapping a word on a phone got a space.
    //
    // The first tap is what says which word. Where the second one lands is an
    // accident of a layout that moved between them, so it does not get a
    // vote — if the first tap was on a word, two taps mean that word.
    if (e.detail === 2) {
      if (!tapWord) return;
  ...
  (async () => {
    ...
    } else if ((zone.emitted_from || {}).test_instrument) {
      // a demonstration has no catalog entry and is not owed one
    } else {
  ...
  const toggleSectionInline = (sec, el, want) => {
    ...
    // case 1 · the coordinate is the section, so this commentary is about the
    // whole verse.
    //
    // A MARK THAT COVERS EVERY WORD DISTINGUISHES NOTHING. It was every word
    // of the section, the same mark a single word gets — which is right when
    // a comment covers three words of twenty-five and says "these", and says
    // nothing at all when it covers all twenty-five. The bar directly above
    // it has already said a commentary is open and whose it is; underlining
    // every word is a second announcement of the same fact, in gold, which on
    // this page means line work. The owner met it on Amos 1:1 and asked what
    // the decoration was, which is what it had become.
    //
    // So the mark is kept for the case it was written for and withheld here,
    // and the words of the section are cleared rather than lit. Every
    // commentary the shelf serves today attaches at the section
    // (check-whose-claim-v1 skips for exactly that reason), so for now this
    // is the whole of what a reader meets; when a word-anchored commentary
    // lands it will not be, and the mark will mean something again.
    el.querySelectorAll(".he-text .wb").forEach((w) => w.classList.remove("c-open"));
  ...
  zone.sections.forEach((sec) => {
    ...
    // distinguishing information is syntax, which lives in the passage.
    // What is drawn here is the case where the boundary is known.
    // Serving part is only honest while we are not claiming to serve the
    // whole, and a claim nobody makes is a claim nobody can rely on. Deleting
    // the withheld part quietly leaves a work that reads as complete and
    // merely short a sentence, which is the flattening defect exactly: a book
    // stripped of its section marks presents as a book that never had any. A
    // petuchah drawn as nothing is a lie about the page, and so is an
    // omission drawn as nothing.
    ...
    const cmPanel = (entries, i0, sectionEl, host, opts = {}) => {
      ...
      const settle = () => {
        ...
        // the page and not two that happen to be adjacent — but only where
        // the mark picks some of the verse out of the rest. A run that covers
        // every word of the verse distinguishes nothing and reads as
        // decoration; the same law as the section case above, which is where
        // it is written out.
        const verseWb = [...(sectionEl.querySelector(".he-text") || sectionEl).querySelectorAll(".wb")]
          .filter((w) => !w.closest(".c-mark-slot"));
        const marks = covers.filter((w) => w.classList && w.classList.contains("wb"));
        if (marks.length && marks.length < verseWb.length) for (const w of marks) w.classList.add("c-open");
      };
      settleRef.fn = settle;
      settle();
