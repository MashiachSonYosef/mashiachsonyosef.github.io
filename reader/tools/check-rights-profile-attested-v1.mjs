...
// its way to is a decision of ours, and the site adds no decisions. The first
// such profile, public domain proved from a source publication year, is
// retired: "technically books can be rearranged and count as new." The
// serving rulings record lists retired profiles; this check holds the site
// to the list.
//
//   L1  the rulings record carries the ruling and at least one retired
//       profile, each naming the ruling that retired it
...
//   L5  the retired profile is still in the catalog under the same id (it is
...
// L1
const why1 = [];
if (!retired.length) why1.push("no retired profile listed");
...
check("L1  the record lists the retired profiles, each under a ruling it carries", why1.length === 0,
  why1.length ? few(why1) : `${retired.length} retired — ${retired.map((r) => `${r.normalized_license_id} (${r.rights_profile_id.slice(0, 12)}…)`).join(", ")}`);
...
if (!bindsPath || !existsSync(LEDGER)) {
...
console.log("  inferences; it says only that the ones the owner retired serve nothing here.");
console.log(bad ? `\n${bad} FAILED` : "\nall checks passed");
