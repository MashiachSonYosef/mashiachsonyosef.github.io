...
for t in tools/check-*.mjs; do
  ...
  case "$name" in
    check-nothing-invented-v1|check-kq-carried-v1|check-attachment-grain-v1|check-corpus-clean-v1|check-store-pinned-v1|check-licence-names-v1|check-page-agrees-with-store-v1|check-clean-address-v1|check-provider-characters-v1|check-sense-split-v1|check-nothing-hand-typed-v1|check-nothing-hard-wired-v1|check-antiquity-tier-v1|check-sealed-layers-v1|check-frame-coverage-v1|check-w-grain-v1|check-nothing-unlanded-v1|check-build-derived-v1|check-docs-name-what-is-here-v1|check-derived-ranges-v1|check-kq-presentation-v1|check-variant-site-v1|check-zone-store-v1|check-door-word-card-v1|check-title-from-c0-v1|check-suggested-title-v1|check-root-card-credit-v1|check-work-attribution-display-v3|check-english-license-v1|check-reference-groups-v1|check-language-admitted-v1|check-apparatus-not-stripped-v1|check-work-attachment-v1|check-no-import-side-effects-v1|check-nothing-welded-v1|check-implicit-maqaf-v1|check-script-admitted-v1|check-ledger-declared-v1|check-workspace-staged-v1) args=() ;;
    *) args=("$URL") ;;
  esac
