...
// The card the word opens: the frame's own letters, each doing its job, and
// saying plainly where one of them stands empty. A mockup may draw the shape
// of a record; it may not draw a record, so M and P are named and left blank
// rather than filled with a source that does not exist.
const LETTER_ROW = (letter, what, value) =>
  `<p class="pc-lrow" title="${esca(what)}"><span class="pc-l">${esca(letter)}</span>`
  + `<span class="pc-lval${value ? "" : " pc-empty"}">${esca(value || "\u2014")}</span></p>`;
const hudHtml = (d) => {
  if (!d.hud || !d.picks) return "";
  const h = d.hud.headings || {}, L = d.hud.letters || {};
  const panes = d.picks.map((k, i) => {
    const c = k.card || {};
    return `<div class="pc-hudpane" data-for="${esca(d.id)}" data-i="${i}"${i === 0 ? "" : " hidden"}>`
      + `<p class="pc-hudword" lang="he" dir="rtl">${esca(k.cells[0].surface)}</p>`
      + LETTER_ROW("D", L.D || "", c.D)
      + (c.R || []).map((r) => LETTER_ROW("R", L.R || "", r)).join("")
      + LETTER_ROW("M", L.M || "", c.M)
      + LETTER_ROW("P", L.P || "", c.P)
      + `<p class="pc-slot">${esca(d.hud.source_slot || "")}</p></div>`;
  }).join("");
  return `<div class="pc-hud">`
    + (d.hud.kq_key ? `<p class="pc-hudkey">${esca(d.hud.kq_key)}</p>` : "")
    + `<p class="pc-hlab">${esca(h.forms || "")} \u00b7 ${d.picks.length}</p>`
    + `<div class="pc-btns">${d.picks.map((k, i) =>
        `<button type="button" class="pc-pick" data-for="${esca(d.id)}" data-i="${i}" aria-pressed="${i === 0}">`
        + `${esca(k.label)}${k.delimiter ? `<span class="pc-delim">${esca(k.delimiter)}</span>` : ""}</button>`).join("")}</div>`
    + panes + `</div>`;
};
...
const nestedHtml = (d) => d.nested ? `<div class="pc-nested">`
  + `<p class="pc-nestlab">and under one atom, an ordinary division</p>`
  ...
  + `</p><p class="pc-incard">${esca(d.nested.why)}</p></div>` : "";
...
const measuresHtml = (d) => d.measures
  ? d.measures.map((m) => `<p class="pc-row"><span class="pc-he" lang="he" dir="rtl">${esca(m.label)}</span>`
    ...
    + `<p class="pc-incard">${esca(d.why_no_numbers_are_shown)}</p>` : "";
