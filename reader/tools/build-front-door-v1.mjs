...
const bookCard = (b) => `    <div class="bookcard">
      ...
      <span class="row">${b.reading
        ? `<span class="en">${esc(b.en)}</span><span class="chip" title="${esc(b.reading.label)}${b.reading.year ? ` \u00b7 ${esc(b.reading.year)}` : ""} — attests this usage. A name is an identification, not licensed expression (FRAME v2.7); who says so, not who permits it.">attested: ${esc(b.reading.label)}</span>`
        : `${plainName(b.slug)
...
const atlasRow = (w) => {
  ...
  if (zi) {
    ...
    return `      <span class="atlas-row built${zi.stamp ? " stamped" : ""}"><span class="col-en"><a class="aw" href="/${name}" title="open this book">${label}</a></span><span class="col-he">${he}</span></span>`;
  }
...
const familySection = (fam) => {
  ...
  const enCell = reading
    ? `<span class="en">${esc(lf.en)}</span><span class="chip" title="commonly force read as — ${esc(reading.label)}${reading.year ? ` · ${esc(reading.year)}` : ""} attests this usage. A name is an identification, not licensed expression (FRAME v2.7); who says so, not who permits it.">attested: ${esc(reading.label)}</span>`
    : (() => {
        const readable = fam.members.map(plainName).filter(Boolean);
        const waits = `listed in the catalog as — ${AWAITS_M}`;
  ...
  return `    <section class="family">
      ...
      <summary>
        <span class="row"><span class="col-en">${enCell}</span><span class="col-he">${famHeadHe(lf)}</span></span>
      </summary>
      <div class="fgroups">
${foldLines.join("\n")}
...
const columnHeads = `    <div class="colheads" aria-hidden="true">
      ...
      <span class="ch he-ch">press the Hebrew for the record behind it</span>
    </div>`;
const sectionsHtml = [
  columnHeads,
