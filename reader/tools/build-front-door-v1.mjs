  ...
  const panes = d.picks.map((k, i) => {
    ...
    return `<div class="pc-hudpane" data-for="${esca(d.id)}" data-i="${i}"${i === 0 ? "" : " hidden"}>`
      ...
      + `<div class="s-pills">${d.picks.map((o, j) =>
          `<button type="button" class="pc-pick" data-for="${esca(d.id)}" data-i="${j}" aria-pressed="${i === j}">`
          + `${esca(o.label)}${o.delimiter ? `<span class="pc-delim">${esca(o.delimiter)}</span>` : ""}</button>`).join("")}</div></div>`
      + `<div class="band b-read"><p class="r-label">${esca(h.routes || "Exact selectable routes")} \u00b7 ${(c.R || []).length}</p>`
      + `<div class="r-pills">${(c.R || []).map((r) => `<button type="button" class="pill" aria-pressed="true">${esca(r)}</button>`).join("")}</div></div>`
      + `</div>`
      + `<div class="d-slot"><p class="pc-d">${esca(c.D)}</p>`
      + `</div>`
      // NOT a .prov line. The reader hides .prov when there is no source, which
      // is right, and this is not a source line — it is a statement that there
      // is none. Wearing the class made it inherit display:none and vanish,
      // which is the correct behaviour applied to the wrong element.
