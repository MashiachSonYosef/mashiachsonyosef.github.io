...
const hudHtml = (d) => {
  ...
  const panes = d.picks.map((k, i) => {
    ...
    return `<div class="pc-hudpane" data-for="${esca(d.id)}" data-i="${i}"${i === 0 ? "" : " hidden"}>`
      + `<div class="band b-q"><p class="r-label">${esca(h.forms || "")} \u00b7 ${d.picks.length}</p>`
      ...
      + (k.divides
          ? `<div class="band b-cut"><p class="r-label">${esca(h.divides || "Complete divisions")} \u00b7 ${k.divides.length}</p>`
            + `<div class="s-pills">${k.divides.map((v, n) =>
                ...
                + `</button>`).join("")}</div>`
            + k.divides.map((v, n) => `<p class="pc-divcells" data-for="${esca(d.id)}-${i}" data-i="${n}"${n === 0 ? "" : " hidden"}>${cellRow(v.cells)}</p>`).join("")
            + `</div>`
          : "")
  ...
  return `<div class="pc-hudwrap" id="hud-${esca(d.id)}" hidden>`
    + (d.hud.kq_key ? `<p class="kq-key">${esca(d.hud.kq_key)}</p>` : "")
    + panes + `</div>`;
};
const glossHtml = (d) => d.gloss_under_word
...
const picksHtml = (d) => {
  ...
  const panes = d.picks.map((k, i) =>
    ...
    + `<p class="pc-cells">${k.cells.map((c) =>
    ...
    + (d.always_shows_both
        ? `<p class="pc-other">${d.picks.filter((o) => o.id !== k.id).map((o) =>
...
const nestedHtml = (d) => d.nested ? `<div class="pc-nested">`
  ...
  + `<p class="pc-cells"><span class="pc-he" lang="he" dir="rtl">${esca(d.nested.of)}</span>`
...
const pocHtml = `  <section id="poc" aria-label="Demonstrations, hand-authored">
    <p class="pc-warn">Typed by hand, from memory — not copied from any text this project holds. Nothing here carries a licence, because nothing here has a record behind it. ${esca(POC.demonstrations.filter((d) => /^Nothing/.test(d.blocks_the_real_thing)).length)} of ${esca(POC.demonstrations.length)} work on the site today; each says below what stands in the way of the rest.</p>
    <p class="pc-law">${esca(POC.THE_LAW_EVERY_CARD_OBEYS)}</p>
${POC.demonstrations.map((d) => `    <article class="pc" data-id="${esca(d.id)}">
