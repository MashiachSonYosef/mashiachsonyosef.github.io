...
const c0Html = (d) => {
  ...
  return `<p class="pc-verse" lang="he" dir="rtl" data-for="${esca(d.id)}">`
    ...
    + `</p><p class="pc-verse-en">${esca(c.english)}</p>`;
};
...
${POC.demonstrations.map((d) => `    <article class="pc" data-id="${esca(d.id)}">
      <p class="pc-head"><span class="pc-kind">${esca(d.kind)}</span><span class="pc-letter">${esca(d.letter)}</span><span class="pc-at">${esca(d.at)}</span>${d.mode ? `<span class="pc-mode">${esca(d.mode)}</span>` : ""}</p>
      <p class="pc-title">${esca(d.title)}</p>
      ${c0Html(d)}
      ${picksHtml(d)}
      ${nestedHtml(d)}
      ${markHtml(d)}
      ...
      <p class="pc-flags">counts ${esca(String(d.counts || "").split(" ")[0])} · ${esca(d.key || "")}</p>
