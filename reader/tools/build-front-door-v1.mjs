...
const atlasRow = (w) => {
  ...
  if (zi) {
    ...
    return `      <span class="atlas-row built${zi.stamp ? " stamped" : ""}">${he}<a class="aw" href="/${name}" title="open this book">${label}</a><span class="au">${au}</span>${zi.stamp ? stampRows(zi.stamp) : ""}</span>`;
  }
...
${page.sections.join("\n")}
  ...
  <!-- The proof stands under the books, not over them. It was the first thing
       on the door: a thousand pixels of count reconciliation, four sections of
       it, before a reader could see one book. The counts are the stamp of
       proof and they belong on the door — under what they are proof OF. -->
${sectionStamp}
