...
// The passage, with the mark standing where it stands. A mark shown alone is
// a glyph; a mark shown inside the line it interrupts is the mark doing its
// work, which is the whole reason to draw one. The site of interest is marked
// so the eye finds it without the surrounding words being dimmed away — the
// rest of the verse is not context, it IS the demonstration.
const passageHtml = (pg) => pg ? `<p class="pc-verse" lang="he" dir="rtl">${
  [pg.before, null, pg.after].map((part, i) => i === 1
    ? `<span class="pc-site">${esc(pg.site)}</span>`
    : (part ? `<span class="pc-plain">${esc(part)}</span>` : "")).filter(Boolean).join(" ")
}</p><p class="pc-verse-en">${esc(pg.english)}</p><p class="pc-ref">${esc(pg.reference || "")}</p>` : "";
const pocBody = (d) => {
  ...
  return d.expands_to
    ? `<p class="pc-row"><span class="pc-role">the mark</span>${heSpan((d.passage && d.passage.site) || "")}<span class="pc-plus">stands for</span>${heSpan(d.expands_to)}</p>`
    : "";
};
