...
// figure; the color reinforces the number and the glyph, never carries it.
const STAMP_CSS = `
...
// about a mark; a role says what a color means. Both are the frame speaking,
// and both are checkable \u2014 so the demonstrations page shows the contract's
// three roles and what wears each. The values are read out of the emitted
...
const CC_TOKEN = { structure: "--gold", reader_selection: "--sel", commentary_attachment: "--shani" };
const CC_WEARS = {
  structure: "verse numbers, the frame of a card, the marks of the page itself",
  reader_selection: "the order position in force, and whatever the reader has chosen",
  commentary_attachment: "where a commentary attaches to a place in the text",
};
...
${CC ? `  <article class="rd" data-id="color-roles">
      <p class="rd-head"><span class="rd-n">\u00b7</span><span class="rd-kind">the color roles</span><span class="rd-letter">${esc(CC.rule_id.split("-").slice(-8).join(" "))}</span><span class="rd-tier rd-carried">carried</span></p>
      <p class="rd-law">${esc(CC.note)}</p>
      <p class="rd-plain">Two faces, both the record's: ${esc(CC.faces.rule)}</p>
      ${Object.entries(CC.roles).map(([role, value]) => `<p class="rd-kindline"><span class="cc-sw" style="background:var(${CC_TOKEN[role] || "--ink"})"></span><span class="rd-kv">${esc(role.replace(/_/gu, " "))}</span><span class="rd-kdrawn">${esc(value.replace(/_/gu, " "))}</span><span class="rd-kn">${esc(CC_TOKEN[role] || "")}</span></p>
      <p class="rd-kat">${esc(CC_WEARS[role] || "")}</p>`).join("\n      ")}
      <p class="rd-kindline"><span class="cc-sw" style="background:var(--shani)"></span><span class="rd-kv">tola\u2019at shani</span><span class="rd-kdrawn">the crimson, on the Hebrew</span><span class="rd-kn">--shani</span></p>
      <p class="rd-kindline"><span class="cc-sw" style="background:var(--sel)"></span><span class="rd-kv">tekhelet</span><span class="rd-kdrawn">the blue, on the English</span><span class="rd-kn">--sel</span></p>
      <p class="rd-kat">A palette to look at rather than a control to add \u2014 the two halves of the name tried on real text, on a link and nothing else. Nothing is stored and no other reader\u2019s page changes.<br><a href="/amos/?palette=tabernacle">Amos 1:1 in tola\u2019at shani and tekhelet</a><br><a href="/amos/">the same verse as it stands</a></p>
      <p class="rd-hide">${esc(CC.basis)} \u00b7 decided ${esc(CC.decided_on)} \u00b7 dies when ${esc(CC.dies_when)}. The swatches above are painted with the page's own tokens, so they cannot drift from what a reader is shown \u2014 if the record and the stylesheet ever disagree, these squares show the stylesheet.</p>
    </article>` : ""}
