...
// row with nothing behind it \u2014 the Hebrew has its key and its licence, the
// chip names who attests the English, the licence names the terms, and the
...
const atlasRow = (w) => {
  ...
  if (zi) {
    ...
    // so a licence sat wherever the taller of the two first cells ended
    // instead of under the reading it releases. The owner, 2026-09-07: line it
    // up by English to Hebrew and glue the licence closer under it, rather
    // than lining up licences too.
...
// about a mark; a role says what a colour means. Both are the frame speaking,
...
const CC_PATH = join(dirname(fileURLToPath(import.meta.url)), "..", "data", "colour-contract-v1.json");
...
const DEF_POSITIONS = [
  ...
  { id: "characters", lab: "characters", live: true, kind: "order", shown: true,
    ...
    say: "Letters match exactly, pointing ignored \u2014 which is how every key on this shelf is made, so this is the catalogue\u2019s own order, undisturbed." },
...
${CC ? `  <article class="rd" data-id="colour-roles">
      <p class="rd-head"><span class="rd-n">\u00b7</span><span class="rd-kind">the colour roles</span><span class="rd-letter">${esc(CC.rule_id.split("-").slice(-8).join(" "))}</span><span class="rd-tier rd-carried">carried</span></p>
