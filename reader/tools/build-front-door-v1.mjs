...
const group24Row = (g) => {
  ...
  const words = named
    ...
    : "";
  const ways = g.files.map((f) => `<a class="aw" href="/${f}" title="open this book">${esc(plainName(f) || f)}</a>`).join("");
...
const familySection = (fam) => {
  ...
  return `    <section class="family">
      ...
      <div class="fgroups">
${foldLines.join("\n")}
${filed24 ? `      <div class="filing-pick" role="group" aria-label="how this shelf is filed"><button type="button" class="fp on" data-filing="39" aria-pressed="true" title="${esc(GROUPING.law)}">${GROUPING.files_39} files</button><button type="button" class="fp" data-filing="24" aria-pressed="false" title="${esc(GROUPING.law)}">${GROUPING.books_24} books</button></div>\n` : ""}${shelfBody}${censusNote}
      </div>
      </details>
    </section>`;
};
