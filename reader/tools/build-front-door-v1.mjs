...
// ---- what a commentary sidecar holds, for whichever row draws it ----------
//
...
//
// This used to be counted inside the loop over the serve plan's works, so a
// commentary only reached the door if its base book was one of them. The
// plan names one; the shelf carries twenty-eight sidecars. Twenty-seven
// commentaries were therefore built, pinned, served at their own addresses
// and mentioned nowhere a reader could find them. The tally is a fact about
// a slug, not about which list a slug happens to be in, so it lives here and
// every row that draws a book asks it.
const COMMENTARY_TALLY = new Map();
const commentaryTally = (slug) => {
  if (COMMENTARY_TALLY.has(slug)) return COMMENTARY_TALLY.get(slug);
  let onWord = 0, onSection = 0, heldLicense = 0, noText = 0, byCoordinate = 0, noCloser = 0, worksCount = 0;
  const side = `${slug}.commentary.bin`;
  if (has(side)) {
    ...
    for (const unit of Object.values(c.units || {})) {
      ...
      for (const e of unit.section || []) {
        ...
        if (e.held === "license") heldLicense += 1;
  ...
  const t = { units: onWord + onSection, onWord, onSection, heldLicense, noText, byCoordinate, noCloser, works: worksCount };
  COMMENTARY_TALLY.set(slug, t);
  return t;
};
...
const STAMP_CSS = `
  ...
  .bookcard .srow.exact .delta, .bookcard .srow.exact .glyph, .atlas-row .srow.exact .delta, .atlas-row .srow.exact .glyph { color:var(--ink-strong); font-weight:600; }
  .bookcard .srow.differs .delta, .bookcard .srow.differs .glyph, .atlas-row .srow.differs .delta, .atlas-row .srow.differs .glyph { color:var(--argaman); font-weight:600; }
  ...
  #sectstamp .sh { font-variant:small-caps; letter-spacing:.12em; color:var(--ink-strong); margin:0 0 .25rem; }
  ...
  #sectstamp tr.exact td.delta, #sectstamp tr.exact td.glyph { color:var(--ink-strong); font-weight:600; }
  #sectstamp tr.differs td.delta, #sectstamp tr.differs td.glyph { color:var(--argaman); font-weight:600; }
...
// THE TWO FILINGS. The corpus lane's grouping ledger (2026-09-07) says which
// of the thirty-nine files this edition ships belong to which of the twenty-
// four books the tradition counts, and states the law in its own words: both
// are claims about where a book divides, and neither is the text. Five of the
// twenty-four hold more than one file — Samuel, Kings, Chronicles, Ezra with
// Nehemiah, and the Twelve — so a reader looking for "kings" and a reader
// looking for "ii kings" are both right, and the door owes them both. It is
// read here, hashed here, and the receipt names the hash; nothing about the
// filing is typed into this file.
const GROUPING_PATH = arg("grouping", "data/book-grouping-v1.json");
...
// So they are their own category on this page, and the narrowest one: a
// gathered name prints only if EVERY word of it is a form the route store
// already carries. Four of the five pass; the fifth is a phrase whose second
// word carries a conjunction no record here attests, and it takes the empty
// slot instead. Computed once, so the row that prints a name and the scrub
// that decides the page may print it can never disagree.
const group24Tokens = (g) => String(g.hebrew_name || "").split(/\s+/).filter(Boolean).map((sfc) => {
  const k = exactK(sfc);
  return { s: sfc, k: k && STORE.routesFor(k) ? k : null };
});
// THE NAME A BOOK IS CALLED, FROM A RECORD. What the door printed in its
// biggest column was the address: the URL slug with its hyphens turned to
// spaces and forced lowercase, so "i-chronicles" read "i chronicles" and
// nothing anywhere said the book is called that. It was the one thing in a
// row with nothing behind it \u2014 the Hebrew has its key and its license, the
// chip names who attests the English, the license names the terms, and the
// name itself was a string derived from a path. The corpus lane's grouping
// ledger names all thirty-nine in English, so the door reads the name from
// there and falls back to the address only where no record names the work.
// The owner's ruling, 2026-09-07.
const LEDGER_EN = new Map(GROUPING
  ? GROUPING.groups.flatMap((g) => (g.file_names || []).map((fn) => [fn.file, fn.en]).filter(([, en]) => en))
  : []);
const bookName = (slug) => LEDGER_EN.get(slug) || plainName(slug);
// WHAT THE SWITCH SAYS ON HOVER, in words a reader has. The ledger's own law
// was printed here verbatim \u2014 "two divisions nobody wrote in the ink (Y)" \u2014
// and (Y) is a letter of our frame, not of anybody's sentence. The ledger
// still says it its own way, where the ledger is read. Here it says the thing
// itself: the same books, counted two ways, and the counting is not the text.
// THE DEFINITION-ORDER TOGGLE'S OWN INPUT. A reader choosing "lexicons of the
// text's own era first" needs to know which source is which, and that is the
// corpus lane's classification, not ours: data/source-corpus-v1.json, emitted
// from its ledger and held to it by check-source-corpus-v1. Only the class
// travels to the page — one word per source id.
const CORPUS_CLASS = (() => {
  const f = "data/source-corpus-v1.json";
  if (!existsSync(f)) return {};
  const rec = JSON.parse(readFileSync(f, "utf8"));
  const out = {};
  for (const [m, v] of Object.entries(rec.witnesses || {})) if (v && v.corpus) out[m] = v.corpus;
  return out;
})();
const FILING_SAYS = GROUPING
  ? `The same books, counted two ways. The tradition counts ${GROUPING.books_24}; this edition ships ${GROUPING.files_39} files, splitting five of them. Neither count is part of the text \u2014 pick the one you read in.`
  : "";
const GROUP_HE_ATTESTED = GROUPING
  ? GROUPING.groups.filter((g) => g.files.length > 1).map((g) => group24Tokens(g))
      .filter((toks) => toks.length && toks.every((t) => t.k))
  : [];
...
// required or a courtesy, so the order falls out of what each license itself
...
// carry it, with their sources and their licenses. Only the test was wrong.
const titleReading = (tokens, en) => {
  ...
  // fullest license first, and among equals the oldest source — the same
  // antiquity rule the rest of the page reads by
  hits.sort((a, c) => {
    const ra = licenseRank((STORE.index.m_sources[a[3]] || {}).licensePosture);
    const rc = licenseRank((STORE.index.m_sources[c[3]] || {}).licensePosture);
...
for (const b of BOOKS) {
  ...
  // its commentary, if any zone carries some for it — one tally, shared with
  // every other row on this page that draws the same book
  const { units, onWord, onSection, heldLicense, noText, byCoordinate, noCloser, works: worksCount } = commentaryTally(b.slug);
  ...
  books.push({ ...b, en: z.work || b.slug, byline: z.byline, sections, words, stamp: z.count_stamp || null,
    ...
    units, onWord, onSection, heldLicense, noText, byCoordinate, noCloser, works: worksCount });
}
...
const bookCard = (b) => `    <div class="bookcard">
      ...
      <span class="row">
        <span class="pair pair-l"><a class="book col-en" href="/${b.slug}">${b.reading
          ? `<span class="en">${esc(b.en)}</span>`
          : plainName(b.slug)
            ? `<span class="en">${esc(plainName(b.slug))}</span>`
            : `<span class="en none" title="${esc(b.slug)}">${NO_PLAIN_NAME}</span>`}</a><span class="col-att">${b.reading
          ...
          : `<span class="of" title="${esc(AWAITS_M)}">awaiting a named source</span>`}</span></span><span class="pair pair-r"><span class="col-he">${b.he
          ? (titleWords(b)
        ...
        <span class="col-lic">${b.heGloss && b.heGlossSrc ? chipHtml(b.heGlossSrc) : ""}</span></span>
      </span>
...
const whereLine = (b) => {
  ...
  if (b.heldLicense) parts.push(`${n(b.heldLicense)} kept off by a license`);
  if (b.noText) parts.push(`${n(b.noText)} named, with no text in the record`);
  return parts.join(" · ");
};
// THE NAME SLOT'S LAW HOLDS HERE TOO. `b.name` is the markup the row above
// already drew for this book — plain letters where a name is recorded, and
// where none is, the absence said in words with the recorded id on the hover.
// Passing a string instead let the entry print a Hebrew title or a raw id as a
// name the day a book without a plain name carried a commentary.
const commentaryLine = (b) => (b.units
  ? `      <a class="sub-book" href="/${b.slug}?c=open"><span class="en">Commentary on ${b.name || esc(b.disp)}</span><span class="of">${n(b.units)} carried${b.works ? ` from ${n(b.works)} work${b.works === 1 ? "" : "s"}` : ""} — ${whereLine(b)}</span></a>`
  : null);
...
const atlasRow = (w) => {
  ...
  if (zi) {
    const tw = titleWords({ heTokens: zi.heTokens, slug: name, disp: bookName(name) || zi.workEn, reading: zi.reading });
    // the same four facts a shelf's heading carries, for a book: who attests
    // the English name, and the license the Hebrew's own reading is released
    ...
    // every later visit. The license in the fourth cell is that reading's own.
    ...
    // THE PAIRS ARE WRAPPED, and on a wide screen the wrappers are not there:
    // display:contents hands the four cells straight to the row's four tracks,
    // so nothing about the desktop columns changes. On a phone the wrappers
    // become the two columns themselves, each stacking its own two cells. That
    // is the whole point \u2014 one grid gives both columns the same row heights,
    // so a license sat wherever the taller of the two first cells ended
    // instead of under the reading it releases. The owner, 2026-09-07: line it
    // up by English to Hebrew and glue the license closer under it, rather
    // than lining up licenses too.
    // A book that carries a commentary says so on its own row, and the entry
    // opens the commentary rather than the top of the book. The shelf is the
    // only list of what is carried, so this asks the shelf and not a plan.
    const cm = commentaryLine({ slug: name, name: label, ...commentaryTally(name) });
    return `      <span class="atlas-row built${zi.stamp ? " stamped" : ""}"><span class="pair pair-l"><span class="col-en"><a class="aw" href="/${name}" title="open this book">${label}</a></span><span class="col-att">${att}</span></span><span class="pair pair-r"><span class="col-he">${he}</span><span class="col-lic">${lic}</span></span></span>${cm ? `\n${cm}` : ""}`;
  }
...
const famHeadHe = (lf) => {
  ...
  // license that reading is released under — never one without the other, so
  // the license gets a column of its own rather than trailing the gloss
...
// A GROUP OF THE TWENTY-FOUR, as a row of the same four cells. The name is not
...
// the store already attests becomes a control; the license cell carries the
// license of whatever reading answers that Hebrew.
const group24Row = (g) => {
  // ALL OF IT OR NONE OF IT. The door may print no character of the text past
  // the titles its records carry, and its own emitter refuses the build when
  // it does \u2014 which is how "\u05e2\u05d6\u05e8\u05d0 \u05d5..." was caught: four of the five gathered
  // names are plain forms the store attests, and the fifth is a phrase whose
  // second word carries a conjunction the store has no record of. So a name
  // prints when every one of its words is attested and otherwise takes the
  // same empty slot every unnamed thing on this door takes. Nothing is
  // trimmed to fit and nothing half-attested is shown.
  const tokens = group24Tokens(g);
  const named = tokens.length > 0 && tokens.every((t) => t.k);
  const key = named ? tokens[0].k : null;
  const gloss = key ? (STORE.glossFor(key).text || "") : "";
  const words = named
    ? tokens.map((t) => `<button type="button" class="fw" data-k="${esc(t.k)}" title="open this word\u2019s own record \u2014 readings oldest source first">${esc(t.s)}</button>`).join(" ")
    : "";
  // THE FILES A GATHERED BOOK IS SHIPPED IN GET A BOX. Run inline after the
  // name they read as more names on the shelf, and the Twelve especially \u2014
  // twelve links wrapping over four lines with nothing saying where the group
  // begins or ends. Boxed, the row says one book, and inside it the files this
  // edition ships it as. The box holds addresses only; the name beside it is
  ...
  return `      <span class="atlas-row built g24"><span class="pair pair-l"><span class="col-en"><span class="g24-name">${esc(g.book24.replace(/-/gu, " "))}</span>${ways}</span>` +
    ...
    `<span class="col-lic">${gloss ? chipHtml(glossSource(key, gloss)) : ""}</span></span></span>`;
};
...
const familySection = (fam) => {
  ...
  // Hebrew, and the license the Hebrew's reading is released under. The owner's
  ...
  // TWO FILINGS, WHERE THE LEDGER COVERS THE SHELF. This shelf's rows are the
  // thirty-nine files; the same books filed as twenty-four are the same rows
  // with five of them gathered. Both are emitted and a switch shows one, so
  // neither filing is the built-in one and a reader can put the door in the
  // shape they count in. The switch rides a body attribute and CSS, never the
  ...
  const rows24 = filed24
    ? [...GROUPING.groups].sort((x, y) => x.book24.localeCompare(y.book24)).map((g) => (g.files.length > 1
        ? group24Row(g)
        : rowsHtml(shelfRows.filter((r) => addressOf(r.atlas.id) === g.files[0])).join("\n"))).join("\n")
    : "";
  const shelfBody = shelfRows.length
    ? (filed24
        ? `      <div class="filing filing-39">\n${rowsHtml(shelfRows).join("\n")}\n      </div>\n      <div class="filing filing-24">\n${rows24}\n      </div>`
        : rowsHtml(shelfRows).join("\n"))
  ...
  return `    <section class="family">
      ...
      <summary>
        <span class="row"><span class="pair pair-l"><span class="col-en">${enName}</span><span class="col-att">${enAtt}</span></span><span class="pair pair-r">${famHeadHe(lf)}</span></span>
        ${shelfRows.length ? `<span class="row n-row">${nLine()}</span>` : ""}
      </summary>
      <div class="fgroups">
${foldLines.join("\n")}
${filed24 ? `      <div class="filing-pick" role="group" aria-label="how this shelf is filed"><button type="button" class="fp on" data-filing="39" aria-pressed="true" title="${esc(FILING_SAYS)}">${GROUPING.files_39} files</button><button type="button" class="fp" data-filing="24" aria-pressed="false" title="${esc(FILING_SAYS)}">${GROUPING.books_24} books</button></div>\n` : ""}${shelfBody}${censusNote}
      </div>
      </details>
    </section>`;
};
...
// standing gloss law.
// N ON THE DOOR. The door said who attests a book's NAME and never said where
// its TEXT came from, which is the wrong half to print on a page whose whole
// claim is that every reading traces to a record. The fact is in each zone's
// own byline, so it is read from there and never typed. If the shelf ever
// stops agreeing, every distinct statement is printed rather than one: a
// collapsed disagreement is the failure this project exists to catch.
const N_ON_THE_SHELF = (() => {
  const seen = new Map();
  for (const slug of ZONE_INFO.keys()) {
    let z;
    try { z = JSON.parse(gunzipSync(readFileSync(join(ZONES, `${slug}.bin`))).toString("utf8")); } catch { continue; }
    const b = String(z.byline || "");
    const version = (b.match(/Version: ([^|]+)/u) || [])[1];
    const license = (b.match(/License: ([^\u00b7|]+)/u) || [])[1];
    if (!version) continue;
    const key = `${version.trim()} \u00b7 ${(license || "").trim() || "no license named"}`;
    seen.set(key, (seen.get(key) || 0) + 1);
  }
  return [...seen.entries()].map(([text, books]) => ({ text, books }));
})();
const nLine = () => {
  if (!N_ON_THE_SHELF.length) return "";
  const total = N_ON_THE_SHELF.reduce((a, x) => a + x.books, 0);
  const one = N_ON_THE_SHELF.length === 1;
  const body = one
    ? esc(N_ON_THE_SHELF[0].text)
    : N_ON_THE_SHELF.map((x) => `${esc(x.text)} (${x.books})`).join(" \u00b7 ");
  const title = one
    ? `the edition every one of these ${total} files was set from, read from each file's own byline and not typed here`
    : `these files were NOT all set from one edition; every edition on the shelf is named rather than one of them standing for the rest`;
  return `<span class="chip n-chip" title="${esc(title)}">text: ${body}</span>`;
};
const DEMO_N = 10;
const demo = (() => {
...
// held. A drawing of a card demonstrates a drawing. The rules of the frame now
...
// THE ROLES, ON THE SAME PAGE AS THE RULES. A rule says what the page does
// about a mark; a channel says who is speaking. Both are the frame speaking,
// and both are checkable \u2014 so the demonstrations page shows the contract's
// five channels and what wears each. The values are read out of the emitted
// stylesheet, never typed here: if the page and the record ever disagree, the
// page is what a reader gets and the swatch says so.
const CC_PATH = join(dirname(fileURLToPath(import.meta.url)), "..", "data", "color-contract-v1.json");
const CC = existsSync(CC_PATH) ? JSON.parse(readFileSync(CC_PATH, "utf8")) : null;
// A channel's token and what actually carries it. v1 had three roles and this
// map had three entries; v2 has five channels, and the two that were added are
// the two a reader looks at most — the corpus's own letters and the reading
// offered for them. commentary_attachment is gone: under v2 a commentary's
// Hebrew is text_as_written like any other work's, and the handle you press to
// open it is frame.
const CC_TOKEN = {
  text_as_written: "--shani",
  reading_as_shown: "--tekhelet",
  our_own_voice: "--argaman",
  structure: "--gold",
  reader_selection: "--sel",
};
const CC_WEARS = {
  text_as_written: "every Hebrew word of the corpus, its titles among them",
  reading_as_shown: "the English under a word, and every reading on its card",
  our_own_voice: "what this page says about itself \u2014 counting notes, rule statements, and every line saying a record holds nothing here",
  structure: "verse numbers, the frame of a card, the marks of the page itself",
  reader_selection: "whatever the reader is holding, and only while they hold it",
};
...
// THE POSITIONS, in one place because two surfaces read them: the pill row a
// reader presses, and the bench at /toggles/ that says what each one is. They
// were a literal inside the page's script, which meant the bench could only
// have been a second copy — and a second copy of a list is a list that drifts.
//
// `live` is whether the evidence a position needs is on this disk. `from` is
// where that evidence is read, named exactly, so a position that claims to
// sort by something can be checked against the thing it sorts by. `kind` is
// order for all seven, meaning exactly one of them is in force at a time and
// "off" is not a state any of them has; a position that admits or refuses
// rather than sorts would be a different kind and would need its own word.
const DEF_POSITIONS = [
  { id: "corpus", lab: "corpus", live: true, kind: "order", shown: true,
    from: "data/source-corpus-v1.json \u00b7 declared_corpus",
    say: "Lexicons of the text\u2019s own era answer first \u2014 the corpus lane\u2019s classification of each source, not ours. The rest keep their own order under them." },
  { id: "masoretic", lab: "masoretic", live: true, kind: "order", shown: true,
    from: "the lattice v12 \u00b7 VOWEL_MATCH \u00b7 projected into <book>.lattice.bin by tools/project-lattice-v12-v1.mjs",
  ...
  { id: "cites", lab: "cites here", live: true, kind: "order", shown: true,
    from: "the lattice v12 \u00b7 names_this_verse \u00b7 projected into <book>.lattice.bin",
  ...
  { id: "characters", lab: "characters", live: true, kind: "order", shown: true,
    from: "data/route-store \u00b7 the key itself",
    say: "Letters match exactly, pointing ignored \u2014 which is how every key on this shelf is made, so this is the catalog\u2019s own order, undisturbed." },
  { id: "vowels-differ", lab: "vowels differ", live: true, kind: "order", shown: true,
    from: "the lattice v12 \u00b7 VOWEL_MISMATCH \u00b7 projected into <book>.lattice.bin",
  ...
  { id: "outside-era", lab: "outside the era", live: true, kind: "order", shown: true,
    from: "data/source-corpus-v1.json \u00b7 declared_corpus, where it is explicitly not the text\u2019s own",
    say: "Lexicons the corpus lane classifies as explicitly NOT of the text\u2019s own era answer first, to let a reader watch the sense move. Unknown is not later: a source with no classification keeps its place." },
  { id: "oldest", lab: "oldest first", live: true, kind: "order", shown: true,
    from: "data/route-store \u00b7 the row\u2019s source year",
    say: "The oldest source answers first. The order this page has always used, and the one it returns to." }
];
...
<script>
// One face since 2026-09-10, so there is nothing to decide and nothing to
// remember. A device's own dark preference is answered by color-scheme in the
// stylesheet, not by a second palette. Anything a device kept from the
// two-faced version is let go of once, so no reader holds a preference for a
// face that no longer exists.
(() => {
  try { localStorage.removeItem("scheme"); } catch { /* a device that remembers nothing still reads */ }
})();
</script>
<style>
${DOOR_CSS}${STAMP_CSS}
...
<body>
<script>
// The button it names now stands inside the page rather than fixed over it,
// so this waits for the page instead of assuming the button is already there.
...
<main
  ...
  <!-- THE TWO CONTROLS SIT IN THE PAGE, not over it. They were fixed to the
       ...
       works this way \u2014 so the two faces of the site now agree. -->
  <div class="topbar">${page.altLink}</div>
  <h1>${page.h1}</h1>
...
${page.sections.join("\n")}
  ...
  <div id="wcard" role="dialog" aria-label="the word&#8217;s own record" hidden>
    ...
    <!-- WHICH READING ANSWERS FIRST. Seven positions, the owner's, and the
         page says plainly which of them it can answer today: a position whose
         evidence is not on this disk is offered dead with the reason on it,
         rather than left off the list as though it had never been asked for. -->
    <div class="def-order" role="group" aria-label="which reading answers first"></div>
  ...
  // THE FILING THE READER COUNTS IN, remembered on this device. The shelf ships
  // both shapes; this only says which one is shown, and it says it on the body
  // so the search box's own [hidden] flag stays the search box's alone.
  var FILING_KEY = "fh.door.filing";
  (function () {
    var picks = [].slice.call(document.querySelectorAll(".filing-pick .fp"));
    if (!picks.length) return;
    function show(v) {
      if (v === "24") document.body.setAttribute("data-filing", "24");
      else document.body.removeAttribute("data-filing");
      picks.forEach(function (b) {
        var on = b.getAttribute("data-filing") === (v === "24" ? "24" : "39");
        b.classList.toggle("on", on);
        b.setAttribute("aria-pressed", String(on));
      });
    }
    var held = null;
    try { held = localStorage.getItem(FILING_KEY); } catch (e) { /* a device that remembers nothing still reads */ }
    if (held === "24") show("24");
    picks.forEach(function (b) {
      b.addEventListener("click", function () {
        var v = b.getAttribute("data-filing");
        show(v);
        try { localStorage.setItem(FILING_KEY, v); } catch (e) { /* the choice still stands on this page */ }
      });
    });
  })();
  ...
  // THE SEVEN POSITIONS, and what each can be answered from today. A position
  // is live when the evidence it needs is on this disk; the four that are not
  // are still listed, dead, with what they wait on written on them. A toggle
  // that silently drops what it cannot do teaches the reader those questions
  // were never asked. (Owner, 2026-09-07: get up as many as you can, even if
  // it starts as binary — it gives us a starting point and a way to remember.)
  var CORPUS = ${JSON.stringify(CORPUS_CLASS)};
  var DEF_KEY = "fh.def.order";
  var DEF_POS = ${JSON.stringify(DEF_POSITIONS)};
  var defOrder = (function () {
    var held = null;
    try { held = localStorage.getItem(DEF_KEY); } catch (e) { /* a device that remembers nothing still reads */ }
    for (var i = 0; i < DEF_POS.length; i++) if (DEF_POS[i].id === held && DEF_POS[i].live) return held;
    return "oldest";
  })();
  ...
  // Every position is a STABLE re-order of the same rows. Nothing is filtered:
  // a reader who asks for the era's own lexicons still sees every reading the
  // catalogue holds, in a different order. Hiding a reading because it sorted
  // late would be the page deciding what the reader may weigh.
  // The row of positions itself, built once per card opening and told how to
  // redraw. A dead position is a real button that says why it is dead: the
  // reader can see the question was asked and what the answer waits on.
  function orderRow(redraw) {
    var row = wcard.querySelector(".def-order");
    if (!row) return;
    row.replaceChildren();
    // THE ONES THAT CAN ANSWER COME FIRST (owner, 2026-09-08: rearrange them
    // rather than cross them out). A struck control reads as withdrawn, and
    // none of these was withdrawn \u2014 they were asked for and their records
    // have not arrived. The book page was corrected that day; this row is the
    // same switch and was left behind, so it is corrected here too.
    DEF_POS.slice().sort(function (a, c) { return (a.live ? 0 : 1) - (c.live ? 0 : 1); }).forEach(function (pos) {
      var b = document.createElement("button");
      b.type = "button";
      b.className = "dfp" + (pos.live ? "" : " waiting") + (pos.id === defOrder ? " on" : "");
      b.textContent = pos.lab;
      b.title = pos.say;
      b.setAttribute("aria-pressed", String(pos.id === defOrder));
      if (!pos.live) { b.disabled = true; b.setAttribute("aria-disabled", "true"); }
      else b.addEventListener("click", function () {
        defOrder = pos.id;
        try { localStorage.setItem(DEF_KEY, pos.id); } catch (e) { /* the choice still stands on this page */ }
        [].slice.call(row.children).forEach(function (x) {
          var on = x.textContent === pos.lab;
  ...
  function sortRows(rows) {
    var r = rows.slice();
    if (defOrder === "characters") return r.sort(function (a, b) { return a[0] - b[0]; });
    if (defOrder === "corpus") {
      return r.sort(function (a, b) {
        var ca = CORPUS[a[3]] === "BIBLICAL" ? 0 : 1, cb = CORPUS[b[3]] === "BIBLICAL" ? 0 : 1;
        return (ca - cb) || byYear(a, b);
      });
    }
    return r.sort(byYear);
  }
  ...
  // license and repaints all three. It is kept per reader, in this browser,
  ...
  // license rode beside the gloss; once the license became the fourth column
  ...
  function openCard(surface, key, book, bookName, bookAtt, bookAttTitle) {
    ...
    Promise.all([idxP, shardOf(key)]).then(function (pair) {
      ...
      return shP.then(function (shard) {
        ...
        var raw = rows;
        // Drawn once per position: a press on the order row re-sorts the same
        // rows and redraws, and the reading the reader had selected stays
        // selected — it moved in the list, it did not change.
        function draw() {
          var ordered = sortRows(raw);
          pills.replaceChildren();
          ...
          // the reading this reader ruled opens selected, so the card agrees
          // with the page instead of arguing with it
          var start = 0;
          if (RULED[key]) for (var q = 0; q < ordered.length; q++) if (ordered[q][1] === RULED[key].text) { start = q; break; }
          paint(ordered, start);
        }
        // THE RECEIPT NAMES THE ORDER IN FORCE, not the one this line was
        // written under. It said "oldest source first" always, which was true
        // while there was only one order and became a false statement about
        // the very list underneath it the moment a reader chose another.
        function stamp() {
          var pos = null;
          for (var i = 0; i < DEF_POS.length; i++) if (DEF_POS[i].id === defOrder) pos = DEF_POS[i];
          wcard.querySelector(".prov").textContent =
            raw.length + " reading" + (raw.length === 1 ? "" : "s") + " \u00b7 " +
            (pos ? pos.lab : defOrder) + " \u00b7 " + (idx.rule_id || "the route store");
        }
        orderRow(function () { draw(); stamp(); });
        draw();
        stamp();
      });
  ...
  function showTip(el) {
    ...
    // the right-edge clamp can go negative when the box is wider than the
    // screen, and a Math.min then chooses the negative and puts the note off
    // the left edge \u2014 which is how a phone got a hover it could only read
    // half of. Eight pixels is the floor, both sides.
    var x = Math.max(8, Math.min(Math.max(8, r.left), window.innerWidth - tr.width - 8));
  ...
  <p class="poc-link"><a href="/toggles/">what the order switch is, and what each position answers from</a></p>
...
const doc = pageDoc({
  ...
  // sorts first, each with its gloss and its license, as a live sample of the
...
const scrubTitles = (text) => {
  ...
  // the gathered books' own names, from the grouping ledger, and only those
  // whose every word the route store attests \u2014 the same narrowing the row
  // itself applies, computed from the same function, so this list cannot
  // allow a character the page does not actually stand behind
  for (const toks of GROUP_HE_ATTESTED) for (const t of toks) { known.push(t.s); known.push(esc(t.s)); if (t.k) known.push(t.k); }
...
if (RD) {
  // The index says the rules of the frame and links each to its passage. It prints
  ...
  const rulePage = (r) => {
    ...
    // A RULE THAT STANDS ON THE BOOKS THEMSELVES. The rules of the frame written
    // first each got a short passage built into a zone of its own, because
    // when they were written no book was readable. Thirty-nine are now, and
    // a rule that fires inside them needs no passage of its own \u2014 it needs
    // the addresses where it fires. Those links open the book at the verse.
    if ((r.kinds || []).length) {
      ...
      <p class="rd-src">${c ? `Carried from ${esc(c.source)}${c.vocabulary ? `, in the source's own vocabulary: ${esc(c.vocabulary)}` : ""}.${c.receipt_note ? ` ${esc(c.receipt_note)}.` : ""}` : ""}</p>
      ${(r.kinds || []).map((k) => `<p class="rd-kindline"><span class="rd-kv">${esc(k.kind.toLowerCase())}</span><span class="rd-kdrawn">${esc(k.drawn)}</span><span class="rd-kn">${esc(k.sites)} sites in ${esc(k.books)} books</span></p>
      <p class="rd-kat">${(k.at || []).map((a) => `<a href="/${esc(a.book)}/?at=${encodeURIComponent(a.label)}">${esc(a.book)} ${esc(a.label)}</a>${a.note ? ` <span class="rd-note">\u2014 ${esc(a.note)}</span>` : ""}`).join("<br>")}</p>`).join("\n      ")}
      ${r.where_two_rules_meet ? `<p class="rd-hide">${esc(r.where_two_rules_meet)}</p>` : ""}
      ${r.may_not_be_hidden ? `<p class="rd-hide">${esc(r.may_not_be_hidden)}</p>` : ""}
    </article>`;
    }
    return `    <article class="rd" data-id="${esc(r.id)}">
      <p class="rd-head"><span class="rd-n">${esc(r.n)}</span><span class="rd-kind">${esc(r.name_en)}</span><span class="rd-letter">${esc(r.letter)}</span><span class="rd-tier rd-${cls}">${esc(word)}</span></p>
...
// THE BENCH, at its own address.
//
// The owner asked for the toggle work to have somewhere of its own rather than
// riding the retired demonstrations page or the books he is auditing. This is
// that page. It says what each position is, whether it can be answered from
// what is on this disk today, and exactly which record answers it \u2014 and it is
// emitted from DEF_POSITIONS, the same array the pill row is built from, so
// the page cannot describe a position the reader does not actually get.
//
// It prints no Hebrew. Every position is a claim about ORDER, and order is
// shown by pressing a word in a real book, not by a specimen typed here.
const posSection = (pos, i) => `    <article class="tg${pos.live ? "" : " tg-wait"}">
      <p class="tg-head"><span class="tg-n">${i + 1}</span><span class="tg-lab">${esc(pos.lab)}</span><span class="tg-badge">${pos.live ? "live" : "waiting"}</span></p>
      <p class="tg-say">${esc(pos.say)}</p>
      <p class="tg-from"><span class="tg-k">answered from</span> ${esc(pos.from)}</p>
    </article>`;
const benchHtml = `<!doctype html>
<html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>The order switch \u00b7 ${SITE_NAME}</title>
<link rel="canonical" href="/toggles/">
<style>${DOOR_CSS}
  .tg { border:1px solid var(--line); border-radius:.5em; padding:.7em .9em; margin:.7em 0; background:var(--panel2); }
  .tg-wait { opacity:.62; }
  .tg-head { margin:0 0 .35em; display:flex; align-items:baseline; gap:.55em; }
  .tg-n { color:var(--faint); font-size:.85em; }
  .tg-lab { color:var(--ink-strong); font-weight:600; }
...
<style>:root{color-scheme:light}
  body{margin:0;min-height:100vh;display:grid;place-items:center;background:#f1e9d8;
  color:#5c5270;font:16px/1.6 Georgia,serif;padding:2rem;text-align:center}
  a{color:#96700f} p{max-width:34rem} .prov{font-size:.82em;color:#7b7290}</style>
</head>
...
<style>:root{color-scheme:light}
  body{margin:0;min-height:100vh;display:grid;place-items:center;background:#f1e9d8;
  color:#5c5270;font:16px Georgia,serif} a{color:#96700f}</style>
</head>
