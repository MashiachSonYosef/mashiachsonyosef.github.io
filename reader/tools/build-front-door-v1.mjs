...
const esca = (t) => esc(String(t == null ? "" : t));
// Whether a string the record hands us is written in Hebrew script. Asked so
// that nothing here has to assume which script a field is in.
const HEBREW_CH = /[\u0590-\u05ff\ufb1d-\ufb4f]/;
// The C0 line. The site is wrapped so a selection can underline part of it
// without the line itself ever changing — that is the law every card obeys.
// The C0 line, rendered once per pick and switched by visibility alone.
//
// The first version built the line in JavaScript, cutting the site string at
// the underline. Two things went wrong and the fence caught both. Cutting a
// string leaves fragments — half of a maqaf word appeared on the page as its
// own run, which is not a word and not a declared string. And the underline
// map itself put Hebrew inside a script literal on a served page, which is
// the exact thing the hand-typing rule forbids, because a literal can become
// output and a comment cannot.
//
// So nothing is cut and no Hebrew reaches JavaScript. Every variant of the
// line is built here, whole, and the script only sets hidden.
const lineVariant = (site, mark) => {
  const t = String(site || ""), m = String(mark || "");
  if (!m || !t.includes(m)) return esca(t);
  if (m === t) return `<span class="pc-under">${esca(t)}</span>`;
  const at = t.indexOf(m);
  return `${esca(t.slice(0, at))}<span class="pc-under">${esca(m)}</span>${esca(t.slice(at + m.length))}`;
};
// A DEMONSTRATION CUT PER WORD. Under the owner's rule 2 a maqaf compound is
// as many C0s as it has words, each its own card and none grouped over them.
// So the site line is not one underline with picks; it is each word drawn as
// its own pressable word, adjacent, with the maqaf riding as ink on the word
// before it exactly as the stream carries it. Nothing is cut here either:
// every surface is a stream row, printed whole.
const wordsLine = (d) => d.words.map((w, n) =>
  `<button type="button" class="pc-open pc-word" data-open="${esca(d.id)}-${n}" lang="he" dir="rtl">${esca(w.surface)}</button>`).join("");
const c0Html = (d) => {
  const c = d.c0 || {};
  const variants = d.words
    ? `<span class="pc-site pc-sitewords" data-i="0">${wordsLine(d)}</span>`
    : (d.picks && d.picks.length ? d.picks : [{ underline: null }])
    .map((k, i) => `<span class="pc-site" data-i="${i}"${i === 0 ? "" : " hidden"}>`
      + (d.hud
          ? `<button type="button" class="pc-open" data-open="${esca(d.id)}" lang="he" dir="rtl">${lineVariant(c.site, k.underline)}</button>`
          : lineVariant(c.site, k.underline))
      + `</span>`)
    .join("");
  return `<p class="pc-verse" lang="he" dir="rtl" data-for="${esca(d.id)}">`
    + (c.before ? `<span class="pc-plain">${esca(c.before)}</span> ` : "")
    + variants
    + (c.after ? ` <span class="pc-plain">${esca(c.after)}</span>` : "")
    + `</p>`
    + (d.gloss_under_word ? `<p class="pc-underline-gloss">${glossHtml(d)}</p>` : "")
    // THE WORDS ARE THE BUTTONS, AND THE PAGE SAYS SO. The reader's own page
    // needs no hint because every word there opens; here the other
    // demonstrations open on one underlined site, so a line under the verse
    // says that each of these words opens its own card before any is pressed.
    + (d.words ? `<p class="pc-press">press any of the ${esca(d.words.length)} underlined words \u2014 each is its own C0 and opens its own card</p>` : "")
    + `<p class="pc-verse-en">${esca(c.english)}</p>`
    + carriedHtml(d);
};
// THE ONE CARRIED DEMONSTRATION WEARS ITS CREDIT. A typed string may carry no
// licence, because nothing stands behind it. A string carried from a record
// must carry the credit its source requires, because something does. The
// line is the binding row's own credit, printed verbatim, with the ledger the
// Hebrew was cut from and the standing of that ledger.
const carriedHtml = (d) => {
  const cf = d.carried_from;
  if (!cf) return "";
  const cr = cf.credit || {};
  return `<p class="pc-carried"><b>Hebrew carried, not typed</b> \u2014 ${esca(cf.ledger)}, rows ${esca(cf.stream_rows)}; ${esca(cf.status)}. ${esca(cf.predecessor)}. ${esca(cf.countersigned)}.</p>`
    + `<p class="pc-carried pc-credit">${esca(cr.line)}${cr.source_url ? ` \u00b7 <a href="${esca(cr.source_url)}" rel="noopener">source</a>` : ""}</p>`;
};
// The picks, and the English that answers to them. Two behaviours, and the
// difference is not decoration: EXCLUDES shows one English because the other
// is wrong here; EMPHASIZES shows every English joined by a plus because
// nothing is excluded and hiding half would assert a separation that is not
// there.
// The gloss under the word in the line, and the card that opens — both
// wearing the reader's own shapes rather than shapes invented for this page,
// because a mockup that looks like something else demonstrates something else.
// The reader's card headings are Transmitted forms and Exact selectable
// routes; its ketiv/qere path already prints a line saying which bracket
// holds which. The one thing this cannot borrow is the licence chip: a chip
// says a record stands behind this reading, and nothing stands behind a typed
// string. The slot is drawn and left saying so.
// THE READER'S OWN CARD, NOT A DRAWING OF ONE.
//
// This was a panel that resembled the card, which demonstrates a panel. The
// card is #hud: a floating overlay with a head that carries the word and a
// close button, a rows region of bands, a d-slot holding the definition and a
// prov line under it holding the source. Those class names and that stylesheet
// are lifted from zone.html verbatim, so what stands here is the shape the
// reader actually draws.
//
// Every state is rendered at build time and the script only sets hidden. No
// Hebrew reaches JavaScript — the same rule that made the line's variants
// pre-rendered, for the same reason: a literal can become output.
const wordHudsHtml = (d) => {
  const h = (d.hud || {}).headings || {};
  return d.words.map((w, n) => {
    const c = w.card || {};
    return `<div class="pc-hudwrap" id="hud-${esca(d.id)}-${n}" hidden>`
      + (n === 0 && d.hud && d.hud.key ? `<p class="card-key">${esca(d.hud.key)}</p>` : "")
      + `<div class="pc-hudpane" data-for="${esca(d.id)}-${n}" data-i="0">`
      + `<div class="head"><b lang="he" dir="rtl">${esca(w.surface)}</b>`
      + `<button type="button" class="pc-x" data-close="${esca(d.id)}-${n}" aria-label="Close">\u00d7</button></div>`
      + `<div class="rows"><div class="band b-read">`
      + `<p class="r-label">${esca(h.routes || "Exact selectable routes")} \u00b7 ${(c.R || []).length}</p>`
      + `<div class="r-pills">${(c.R || []).map((r) => `<button type="button" class="pill" aria-pressed="true">${esca(r)}</button>`).join("")}</div>`
      + `</div></div>`
      + `<div class="d-slot"><p class="pc-d">${esca(c.D)}</p></div>`
      + `<p class="pc-noprov">${esca(c.M || (d.hud || {}).source_slot || "")}</p>`
      + `</div></div>`;
  }).join("");
};
const hudHtml = (d) => {
  if (d.words) return wordHudsHtml(d);
  if (!d.hud || !d.picks) return "";
  const h = d.hud.headings || {};
  const cellRow = (cells) => cells.map((c) =>
    `<span class="pc-cell${c.focus === false ? " pc-dim" : ""}">`
    + `<span class="pc-he" lang="he" dir="rtl">${esca(c.surface)}</span>`
    + `<span class="pc-en">${esca(c.english)}</span></span>`).join('<span class="pc-plus">+</span>');
  const panes = d.picks.map((k, i) => {
    const c = k.card || {};
    // THE LATTICE STANDS ABOVE THE WORD. Layer one asks which word the C0
    // holds; the word in focus follows from the answer, so the question comes
    ...
    return `<div class="pc-hudpane" data-for="${esca(d.id)}" data-i="${i}"${i === 0 ? "" : " hidden"}>`
      ...
      + `<div class="s-pills" dir="rtl">${d.picks.map((o, j) =>
          ...
          + (o.pill_cells
              ...
              // delimiter, wearing the name of what that delimiter holds. The
              // record decides which script it writes that in, so the direction
              // is read off the string rather than assumed.
              ? (HEBREW_CH.test(o.pill)
                  ? `<span lang="he" dir="rtl">${esca(o.pill)}</span>`
                  : `<span class="pc-term">${esca(o.pill)}</span>`)
              : `${esca(o.label)}${o.delimiter ? `<span class="pc-delim">${esca(o.delimiter)}</span>` : ""}`)
          + `</button>`).join("")}</div></div>`)
      + `<div class="head"><b lang="he" dir="rtl">${esca(k.cells.length === 1 ? k.cells[0].surface : (k.underline || ""))}</b>`
      + `<button type="button" class="pc-x" data-close="${esca(d.id)}" aria-label="Close">\u00d7</button></div>`
      + (k.divides
          ? `<div class="band b-cut"><p class="r-label">${esca(h.divides || "Complete divisions")} \u00b7 ${k.divides.length}</p>`
            + `<div class="s-pills" dir="rtl">${k.divides.map((v, n) =>
                `<button type="button" class="pc-div" data-for="${esca(d.id)}-${i}" data-i="${n}" aria-pressed="${n === 0}">`
                + v.cells.map((x) => `<span lang="he" dir="rtl">${esca(x.surface)}</span>`).join(' <span class="pc-plus">+</span> ')
                + `</button>`).join("")}</div>`
            + k.divides.map((v, n) => `<p class="pc-divcells" dir="rtl" data-for="${esca(d.id)}-${i}" data-i="${n}"${n === 0 ? "" : " hidden"}>${cellRow(v.cells)}</p>`).join("")
            + `</div>`
          : "")
      + `<div class="rows"><div class="band b-read">`
      + `<p class="r-label">${esca(h.routes || "Exact selectable routes")} \u00b7 ${(c.R || []).length}</p>`
      ...
      + `</div></div>`
      + `<div class="d-slot"><p class="pc-d">${esca(c.D)}</p></div>`
      + `<p class="pc-noprov">${esca(d.hud.source_slot || "")}</p>`
      + `</div>`;
  }).join("");
  return `<div class="pc-hudwrap" id="hud-${esca(d.id)}" hidden>`
    + (d.hud.key ? `<p class="card-key">${esca(d.hud.key)}</p>` : "")
    + panes + `</div>`;
};
const glossHtml = (d) => d.words
  ? `<span class="pc-underword">${d.gloss_under_word.map((g) => esca(g)).join(' <span class="pc-plus">+</span> ')}</span>`
  : d.gloss_under_word
  ? d.picks.map((k, i) => `<span class="pc-underword" data-for="${esca(d.id)}" data-i="${i}"${i === 0 ? "" : " hidden"}>`
      + `${esca(d.gloss_under_word[k.id] || "")}</span>`).join("")
  : "";
const picksHtml = (d) => {
  if (d.hud) return hudHtml(d) + (d.in_the_card ? `<p class="pc-incard">${esca(d.in_the_card)}</p>` : "");
  if (!d.picks) return "";
  const both = d.always_shows_both ? " data-both=\"1\"" : "";
  const buttons = d.picks.map((k, i) =>
    `<button type="button" class="pc-pick" data-for="${esca(d.id)}" data-i="${i}" aria-pressed="${i === 0}">`
    + `${esca(k.label)}${k.delimiter ? `<span class="pc-delim">${esca(k.delimiter)}</span>` : ""}</button>`).join("");
  const panes = d.picks.map((k, i) =>
    `<div class="pc-pane" data-for="${esca(d.id)}" data-i="${i}"${i === 0 ? "" : " hidden"}>`
    + `<p class="pc-belongs">${d.mode === "EXCLUDES"
        ? `this English is the <b>${esca(k.label)}</b> form only`
        : `in focus: <b>${esca(k.label)}</b> — nothing is excluded`}</p>`
    + `<p class="pc-cells" dir="rtl">${k.cells.map((c) =>
        `<span class="pc-cell${c.focus === false ? " pc-dim" : ""}"><span class="pc-he" lang="he" dir="rtl">${esca(c.surface)}</span>`
        + `<span class="pc-en">${esca(c.english)}</span></span>`).join('<span class="pc-plus">+</span>')}</p>`
    + (d.always_shows_both
        ? `<p class="pc-other" dir="rtl">${d.picks.filter((o) => o.id !== k.id).map((o) =>
            `<span class="pc-otherlab">and equally</span>` + o.cells.map((c) =>
              `<span class="pc-cell"><span class="pc-he" lang="he" dir="rtl">${esca(c.surface)}</span>`
              + `<span class="pc-en">${esca(c.english)}</span></span>`).join('<span class="pc-plus">+</span>')).join("")}</p>`
        : "")
    + `</div>`).join("");
  return `<div class="pc-picks"${both} data-mode="${esca(d.mode || "")}" data-id="${esca(d.id)}">`
    + `<div class="pc-btns">${buttons}</div>${panes}</div>`
    + (d.in_the_card ? `<p class="pc-incard">${esca(d.in_the_card)}</p>` : "");
};
// A maqaf's atom is a word, and a word carries prefixes, so its lattice sits
// over an ordinary COMPspan instead of replacing it.
const nestedHtml = (d) => d.nested ? `<div class="pc-nested">`
  + `<p class="pc-nestlab">under one atom</p>`
  + `<p class="pc-cells" dir="rtl"><span class="pc-he" lang="he" dir="rtl">${esca(d.nested.of)}</span>`
  + `<span class="pc-arrow">→</span>`
  + d.nested.into.map((c) => `<span class="pc-cell"><span class="pc-he" lang="he" dir="rtl">${esca(c.surface)}</span>`
      + `<span class="pc-en">${esca(c.english)}</span></span>`).join('<span class="pc-plus">+</span>')
  + `</p></div>` : "";
const markHtml = (d) => d.expands_to
  ? `<p class="pc-row"><span class="pc-role">the mark</span>`
    + `<span class="pc-he" lang="he" dir="rtl">${esca((d.c0 || {}).site)}</span>`
    + `<span class="pc-plus">stands for</span>`
    + `<span class="pc-he" lang="he" dir="rtl">${esca(d.expands_to)}</span></p>` : "";
const measuresHtml = (d) => d.measures
  ? d.measures.map((m) => `<p class="pc-row"><span class="pc-he" lang="he" dir="rtl">${esca(m.label)}</span>`
      + `<span class="pc-en">${esca(m.english)}</span></p>`).join("")
    + `<p class="pc-row"><span class="pc-role">state</span><span class="pc-state">${esca(d.the_state_shown_here)}</span></p>`
    + `<p class="pc-slot">${esca(d.why_no_numbers_are_shown)}</p>` : "";

const pocHtml = `  <section id="poc" aria-label="Demonstrations, hand-authored">
    <p class="pc-warn">Typed by hand, from memory — not copied from any text this project holds — except where an article says on its face that its Hebrew is carried from a record. A typed string carries no licence, because nothing stands behind it; a carried one wears the credit its source requires. ${esca(POC.demonstrations.filter((d) => /^Nothing/.test(d.blocks_the_real_thing)).length)} of ${esca(POC.demonstrations.length)} work on the site today; each says below what stands in the way of the rest.</p>
    <p class="pc-law">${esca(POC.THE_LAW_EVERY_CARD_OBEYS)}</p>
${POC.demonstrations.map((d) => `    <article class="pc" data-id="${esca(d.id)}"${d.carried_from ? ' data-carried="1"' : ""}>
      <p class="pc-head"><span class="pc-kind">${esca(d.kind)}</span><span class="pc-letter">${esca(d.letter)}</span><span class="pc-at">${esca(d.at)}</span></p>
      ${d.title ? `<p class="pc-title">${esca(d.title)}</p>` : ""}
      ${c0Html(d)}
      ${picksHtml(d)}
      ${nestedHtml(d)}
      ${markHtml(d)}
      ${measuresHtml(d)}
      <p class="pc-says">${esca(d.the_card_says)}</p>
      ${d.plainly ? `<p class="pc-flags">${esca(d.plainly)}</p>` : ""}
      <p class="pc-block">${/^Nothing/.test(d.blocks_the_real_thing) ? "<b>live on this site</b> — " : "<b>not yet</b> — "}${esca(d.blocks_the_real_thing)}</p>
    </article>`).join("\n")}
    <script>
    // Visibility only. No Hebrew reaches this script and nothing is cut at
    // runtime — every variant of the line was rendered whole at build time.
    (function () {
      document.querySelectorAll(".pc-pick").forEach(function (btn) {
        btn.addEventListener("click", function () {
          var id = btn.dataset.for, i = btn.dataset.i;
          document.querySelectorAll('.pc-pick[data-for="' + id + '"]').forEach(function (b) {
            b.setAttribute("aria-pressed", String(b.dataset.i === i));
          });
          document.querySelectorAll('.pc-pane[data-for="' + id + '"]').forEach(function (p) {
            p.hidden = p.dataset.i !== i;
          });
          var line = document.querySelector('.pc-verse[data-for="' + id + '"]');
          if (line) line.querySelectorAll(".pc-site").forEach(function (v) { v.hidden = v.dataset.i !== i; });
          document.querySelectorAll('.pc-underword[data-for="' + id + '"]').forEach(function (g) { g.hidden = g.dataset.i !== i; });
          document.querySelectorAll('.pc-hudpane[data-for="' + id + '"]').forEach(function (h) { h.hidden = h.dataset.i !== i; });
          var w = document.getElementById("hud-" + id); if (w) w.hidden = false;
        });
      });
      document.querySelectorAll(".pc-div").forEach(function (btn) {
        btn.addEventListener("click", function () {
          var g = btn.dataset.for, i = btn.dataset.i;
          document.querySelectorAll('.pc-div[data-for="' + g + '"]').forEach(function (b2) {
            b2.setAttribute("aria-pressed", String(b2.dataset.i === i));
          });
          document.querySelectorAll('.pc-divcells[data-for="' + g + '"]').forEach(function (r) {
            r.hidden = r.dataset.i !== i;
          });
        });
      });
      document.querySelectorAll(".pc-open").forEach(function (btn) {
        btn.addEventListener("click", function () {
          ...
          document.querySelectorAll(".pc-hudwrap").forEach(function (w) { w.hidden = w.id !== "hud-" + id; });
        });
      });
      document.querySelectorAll(".pc-x").forEach(function (btn) {
        btn.addEventListener("click", function () {
          var w = document.getElementById("hud-" + btn.dataset.close); if (w) w.hidden = true;
        });
      });
    })();
    </script>
  </section>`;
const pocLink = `  <p class="poc-link"><a href="/demonstrations/">what each part of the frame looks like when it works</a> \u2014 hand-drawn mockups, not text from any book, but for one verse of Ruth cut from the corpus lane's record</p>`;
